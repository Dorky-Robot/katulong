 ...
 * When katulong is started from within another tool (e.g. Claude Code), the tmux
 * server inherits env vars like CLAUDECODE that pollute all sessions/panes.
 * This must run once at startup after confirming tmux is available.
 */
export async function cleanTmuxServerEnv() {
  const removals = [];
  for (const varName of SENSITIVE_ENV_VARS) {
    // -gr removes from the global environment; -r removes from server default
