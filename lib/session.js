...
export class Session {
  ...
  constructor(name, tmuxName, options = {}) {
    ...
    this._lastSnapshotLines = null; // previous frame's lines (for diffing)
    this._lastCursorPos = null;     // previous cursor "x,y" string
  }

  /**
   * Diff two line arrays. Returns array of [row, content] pairs for changed rows.
   * Returns null if identical.
   */
  static _diffLines(prev, curr) {
    const changed = [];
    const max = Math.max(curr.length, prev ? prev.length : 0);
    for (let i = 0; i < max; i++) {
      if (!prev || curr[i] !== prev[i]) {
        changed.push([i, curr[i] || ""]);
      }
    }
  ...
  /**
   * Capture a single frame and emit changed lines.
   */
  _captureFrame() {
    if (!this._onSnapshot) return;
    let paneData = null, cursorStr = null, pending = 2;
    const done = () => {
      if (--pending > 0) return;
      if (!paneData) return;
      ...
      const changed = Session._diffLines(prev, lines);
      const cursorChanged = cursorStr !== this._lastCursorPos;
      this._lastCursorPos = cursorStr;
      ...
      if (this._onSnapshot) {
        this._onSnapshot(this.name, { rows: changed || [], cx, cy });
      }
    };
    ...
    execFile("tmux", ["display-message", "-t", this.tmuxName, "-p", "#{cursor_x},#{cursor_y}"],
      { timeout: 5000 }, (err, stdout) => {
        if (!err && stdout) cursorStr = stdout.trim();
        done();
      });
  }
