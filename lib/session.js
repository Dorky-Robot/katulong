...
export class Session {
  ...
  /**
   * Fetch recent pane content directly from tmux — the visible screen
   * plus `lines` of scrollback as plain text. Use this when callers need
   * a summary-style window that survives server restarts: the RingBuffer
   * starts empty on restore (tmux control mode only streams NEW output),
   * but tmux itself has kept the pane's history.
   *
   * Returns "" on failure; callers decide what to do with an empty read.
   */
  async captureScrollback(lines = 500) {
    if (!this.tmuxName) return "";
    try {
      return await captureRecentPane(this.tmuxName, lines);
    } catch {
      return "";
    }
  }
