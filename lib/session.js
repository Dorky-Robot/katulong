...
export class Session {
  ...
  resize(cols, rows) {
    ...
    //
    // If output was received within the last OUTPUT_IDLE_GATE_MS, defer
    // the resize until output settles.  The deferred resize replaces any
    // previously pending one (only the latest dimensions matter).
    const OUTPUT_IDLE_GATE_MS = 50;
    const sinceLast = Date.now() - this._lastOutputAt;
    if (sinceLast < OUTPUT_IDLE_GATE_MS) {
      clearTimeout(this._resizeTimer);
      this._resizeTimer = setTimeout(() => {
        this._resizeTimer = null;
        this._applyResize(cols, rows);
      }, OUTPUT_IDLE_GATE_MS - sinceLast);
      return;
    }
