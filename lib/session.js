...
export class Session {
  ...
  attachControlMode(cols, rows) {
    ...
    // tmuxSocketArgs() prepends `-L <name>` when KATULONG_TMUX_SOCKET is set,
    // routing the control-mode client to the same isolated tmux server that
    // tmuxExec() uses. Without this, the e2e test server would start sessions
    // on the isolated socket but then fail to attach to them.
