...
export class Session {
  ...
  _drainParserState() {
    ...
    if (payloadTail) {
      ...
      if (this._headless) this._headless.write(payloadTail);
      if (this._onData) {
  ...
  async serializeScreen() {
    try {
      ...
      if (!this._headless) return ""; // disposed during flush await
   ...
   *
   * Returns `{ hash, seq }` where `seq` is the byte position in the output
   * stream that the headless reflects at the moment the buffer was read.
   * Pairing the two prevents a Lamport-style ordering bug: the client must
   ...
   * and trigger a spurious resync.
   *
   * `seq` is captured AFTER the flush await but BEFORE reading the buffer:
   ...
   * that has been pushed (handleOutput writes to outputBuffer and headless
   * in the same synchronous block, so they cannot disagree mid-await).
   *
   * Wrapped in try/catch and double-checked after the flush await because
   * `scheduleIdleCheck` in session-manager calls this from a setTimeout,
   * and the session can be killed (disposing `_headless`) during the yield.
   * Without the post-await guard, `buffer.active` would throw and the
   * rejection would be unhandled (the timer callback has no catcher).
   */
  async screenFingerprint() {
    try {
      if (!this._headless) return { hash: 0, seq: 0 };
      await new Promise(resolve => this._headless.write("", resolve));
      if (!this._headless) return { hash: 0, seq: 0 }; // disposed during flush await
