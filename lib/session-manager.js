...
export function createSessionManager({ bridge, shell, home }) {
  ...
  // Output coalescing: debounce per-session output to keep TUI screen updates
  // atomic.  A short idle timer (2ms) flushes when output stops arriving,
  // a hard max-delay timer (32ms) bounds worst-case latency for continuous
  // streams.  This prevents partial TUI redraws from reaching the client as
  // separate WebSocket messages rendered in different animation frames.
  const outputCoalesce = new Map();  // sessionName -> { data, seq, idleTimer, maxTimer }
  const OUTPUT_IDLE_MS = 2;    // flush after 2ms of silence (debounce)
  const OUTPUT_MAX_MS = 32;    // hard deadline — never buffer longer than this
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      onData: (sessionName, data) => {
        ...
        if (existing) {
          existing.data += data;
          // Reset idle timer (debounce) — wait for output to stop before flushing
          // so TUI screen updates stay atomic. Max timer is unchanged.
          if (existing.idleTimer) clearTimeout(existing.idleTimer);
          existing.idleTimer = setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_IDLE_MS);
        } else {
          const session = sessions.get(sessionName);
          // totalBytes already includes this data (push happened before onData),
          ...
          outputCoalesce.set(sessionName, {
            ...
            idleTimer: setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_IDLE_MS),
            maxTimer: setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_MAX_MS),
          });
        }
      },
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      let buffer = "";
      // Flush pending coalesced output and snapshot totalBytes before the
      // async capture window, so the seq value returned to the caller is
      // exactly the byte offset where the next live output chunk starts.
      // Output arriving during captureScrollback will have seq >= this
      // value and be delivered via the normal bridge path.
      flushOutputBuffer(name);
