...
export function createSessionManager({ bridge, shell, home }) {
  ...
  function resetResyncTimer(sessionName) {
    const existing = resyncTimers.get(sessionName);
    if (existing) clearTimeout(existing);
    const timer = setTimeout(() => {
      resyncTimers.delete(sessionName);
      fireResync(sessionName);
    }, RESYNC_IDLE_MS);
    timer.unref();
    resyncTimers.set(sessionName, timer);
  }
  ...
  return {
    ...
    async subscribeClient(clientId, sessionName) {
      ...
      if (session.alive) buffer = session.serializeScreen();
      const seq = session.outputBuffer.totalBytes;
