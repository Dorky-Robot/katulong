...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false, persistent = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      onExit: (sessionName, exitCode) => {
        ...
        // Persistent sessions (e.g. dispatch) survive shell exit so the user
        // can scroll back through the output. Normal sessions auto-delete.
        if (!persistent) deleteSession(sessionName);
      },
    });
  ...
  async function spawnSession(name, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, cwd = null, { persistent = false } = {}) {
    ...
    const promise = doSpawnSession(name, cols, rows, cwd, { persistent }).finally(() => {
  ...
  async function doSpawnSession(name, cols, rows, cwd, { persistent = false } = {}) {
    ...
    const session = await adoptSession(name, tmuxName, cols, rows, { persistent });

    sessions.set(name, session);
    detachedNames.delete(tmuxName);
    log.info("Session created", { session: name, tmux: tmuxName, reattached: exists, persistent });
    return session;
  }
  ...
  return {
     ...
     * @returns {Promise<{ name: string } | { error: string }>}
     */
    async createSession(name, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, copyFrom = null, { persistent = false } = {}) {
      if (aliveSessionCount() >= MAX_SESSIONS) return { error: `Maximum session limit (${MAX_SESSIONS}) reached` };
      ...
      await spawnSession(name, cols, rows, cwd, { persistent });
      scheduleSave();
      return { name };
    },
