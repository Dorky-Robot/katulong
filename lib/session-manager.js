...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
     ...
     */
    async attachClient(clientId, sessionName, cols, rows, { skipResize = false } = {}) {
      ...
      cancelNotification(name);
      if (session.alive) {
        // Skip resize for cached switches — the client already has the
        // right content and will send its own resize via onResize.
        // Resizing here triggers tmux redraws that go to the RingBuffer,
        // causing duplicate content on pull.
        if (!skipResize) session.resize(cols, rows);
        buffer = session.serializeScreen();
      }
      cancelNotification(name);
     ...
     * @param {string} clientId
     * @param {string} sessionName
     * @param {number} [cols] - Terminal width (if provided, resize before snapshot)
    ...
    async subscribeClient(clientId, sessionName, cols, rows) {
      ...
      if (session.alive) {
        // Resize the headless xterm (and PTY) to match the carousel card
        // dimensions before serializing. Without this, the snapshot wraps
        // at the wrong column width, causing garbled text on the client.
