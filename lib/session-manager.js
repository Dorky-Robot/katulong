...
export function createSessionManager({ bridge, shell, home }) {
  ...
  function detachControlProc(session) {
    if (session.controlProc) {
      try { session.controlProc.stdin.end(); session.controlProc.kill(); } catch (err) {
        log.debug("Control proc already dead", { session: session.name, error: err.message });
      }
    }
  }
  ...
  return {
    ...
    async listTmuxSessions() {
      ...
      const unmanaged = tmuxSessions.filter(name => !isAlreadyManaged(name));
      return { sessions: unmanaged };
    },
    ...
    async adoptTmuxSession(tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS) {
      ...
      // Validate name: reject tmux target specifiers (: . /) and enforce length limit
      if (tmuxName.length > 128 || /[:.\/]/.test(tmuxName)) {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Resize to client dimensions — this also forces tmux to repaint
      // the screen, ensuring the prompt and current content are visible
      // even if the initial resize message races with attach.
      if (session.alive) {
        session.resize(cols, rows);
      }
      // Use capture-pane for the current visible screen state instead of
      // replaying the full RingBuffer history. The RingBuffer contains raw
      // %output chunks with cursor-positioning escapes that cause doubled
      // content when replayed (especially for TUI apps like Claude Code).
      // capture-pane gives a clean snapshot of what's currently on screen.
      let buffer = null;
      if (session.alive) {
        buffer = await captureScrollback(session.tmuxName);
      }
      return { buffer: buffer || "", alive: session.alive };
    },
