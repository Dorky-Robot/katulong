...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  async function discoverSessions() {
    ...
    for (const tmuxName of tmuxSessions) {
      // Skip sessions we already manage
      const alreadyManaged = [...sessions.values()].some(s => s.tmuxName === tmuxName);
      if (alreadyManaged) continue;

      // Use the tmux session name as the katulong session name
      const name = tmuxName;
      ...
      try {
        ...
        session.attachControlMode(120, 40);
  ...
  return {
     ...
     */
    async createSession(name, cols = 120, rows = 40, copyFrom = null) {
