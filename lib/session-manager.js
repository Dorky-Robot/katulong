...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  return {
    ...
    async restoreSessions() {
      ...
      const tmuxSessions = await tmuxListSessions();
      const orphans = tmuxSessions.filter(name => !isAlreadyManaged(name));
      if (orphans.length > 0) {
        log.info("Auto-adopting unmanaged tmux sessions", { count: orphans.length, sessions: orphans });
        await Promise.allSettled(
          orphans.map(async (tmuxName) => {
            if (aliveSessionCount() >= MAX_SESSIONS) {
              log.warn("Skipping auto-adopt — session limit reached", { tmuxName });
              return;
            }
            try {
              const session = await adoptSession(tmuxName, tmuxName, DEFAULT_COLS, DEFAULT_ROWS, { external: true });
              // Re-check after async work (guards against parallel adopt race)
              if (isAlreadyManaged(tmuxName)) {
                session.detach();
                return;
              }
