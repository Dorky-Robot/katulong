...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async subscribeClient(clientId, sessionName, cols, rows) {
      ...
      if (session.alive) {
        // Don't resize before serialize — resizing triggers tmux redraws
        // that race with serialization, causing garbled snapshots.
        // Cols are always 80 (FIXED_COLS), so row differences don't
        // affect content layout. The PTY is already sized by the primary
        // client's attach.
        buffer = session.serializeScreen();
      }
      cancelNotification(sessionName);
