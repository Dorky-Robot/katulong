...
const OUTPUT_IDLE_MS = 2;    // debounce: flush after 2ms of silence
...
export function createSessionManager({ bridge, shell, home }) {
  ...
  function scheduleIdleFlush(sessionName) {
    return setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_IDLE_MS);
  }
    ...
    const session = new Session(name, tmuxName, {
      ...
      onData: (sessionName, data) => {
        ...
        if (existing) {
          ...
          existing.idleTimer = scheduleIdleFlush(sessionName);
        } else {
          ...
          outputCoalesce.set(sessionName, {
            ...
            idleTimer: scheduleIdleFlush(sessionName),
