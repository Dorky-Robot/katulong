...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
     ...
     * If no other clients remain attached to the session, detach control mode
     * (keeps tmux session alive but frees the "attached" status in tmux ls).
     * @param {string} clientId
     */
    detachClient(clientId) {
      ...
      // Check if any other clients are still attached to this session
      const sessionName = info.session;
      let hasOtherClients = false;
      for (const [, other] of clients) {
        if (other.session === sessionName) { hasOtherClients = true; break; }
      }

      if (!hasOtherClients) {
        const session = sessions.get(sessionName);
        if (session) {
          log.info("Last client detached, releasing control mode", { session: sessionName });
          session.detach();
          detachedNames.add(session.tmuxName);
          sessions.delete(sessionName);
          bridge.relay({ type: "session-removed", session: sessionName });
        }
      }
    },
