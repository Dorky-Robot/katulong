...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Don't replay raw RingBuffer output — TUI apps use cursor-positioning
      // escapes that cause garbled/doubled content when replayed. Instead,
      // force tmux to do a full screen redraw via control mode.
      if (session.alive) {
        // Force a full repaint: resize to 1 col smaller then back, which
        // makes tmux redraw the entire visible screen via %output.
