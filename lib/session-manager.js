...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    deleteSession(name, { detachOnly = false } = {}) {
      ...
      // Cancel any pending data-available notification before removing
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      cancelNotification(name);
      if (session.alive) {
        // Resize FIRST so the terminal redraws at the correct dimensions
        // before we capture. This avoids the race where resize triggers
        // new output after the capture, causing mixed frames.
        session.resize(cols, rows);

        // Brief pause to let any resize-triggered redraws settle into
        // the RingBuffer before we snapshot.
        await new Promise(r => setTimeout(r, 50));

        // Capture visible pane only (single coherent frame). Scrollback
        // history is not sent — it mixes rendered text with raw escape
        // sequences from the RingBuffer, causing garbled output when a
        // TUI app is actively redrawing.
        const snapshot = await captureVisiblePane(session.tmuxName);
        if (snapshot) buffer = snapshot;
      }
      // Snapshot seq immediately after capture — no more async gaps
      // where output can sneak in between capture and seq.
      cancelNotification(name);
      const seq = session.outputBuffer.totalBytes;
      const cursor = session.alive ? await getCursorPosition(session.tmuxName) : null;
      return { buffer, alive: session.alive, seq, cursor };
    },
    ...
    async subscribeClient(clientId, sessionName) {
      ...
      // Capture scrollback so the subscribed terminal shows existing content
      let buffer = "";
      ...
      // Snapshot seq AFTER capture — same reasoning as attachClient
      cancelNotification(sessionName);
      const seq = session.outputBuffer.totalBytes;
