...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  const pendingSpawns = new Map(); // name -> Promise<Session> (serializes concurrent spawns)
  ...
  async function spawnSession(name, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, cwd = null) {
    ...
    pendingSpawns.set(name, promise);
    return promise;
  }
  ...
  return {
    ...
    async adoptTmuxSession(tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS) {
      ...
      const pending = pendingSpawns.get(tmuxName);
      if (pending) {
        await pending;
      ...
      const promise = doAdoptTmuxSession(tmuxName, cols, rows).finally(() => {
        pendingSpawns.delete(tmuxName);
      });
      pendingSpawns.set(tmuxName, promise);
      return promise;
    },
