...
export function createSessionManager({ bridge, shell, home }) {
  ...
  // Snapshot-based output: sessions capture their visible pane at ~30fps
  // during activity and relay snapshots directly to subscribed clients
  // via the transport bridge. No client-side cursor or pull logic needed.
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      external,
      onSnapshot: (sessionName, snapshot) => {
        const { type: snapshotType, ...rest } = snapshot;
        bridge.relay({ type: "snapshot", session: sessionName, snapshotType, ...rest });
      },
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      let buffer = "";
      // Cancel pending notification before the async capture window
      // so totalBytes is up-to-date.
            if (session.alive) {
        // Capture visible screen only — snapshot model replaces scrollback.
        // Each frame shows the current screen state; no history needed.
