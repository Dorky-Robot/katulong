...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Use capture-pane for a clean snapshot of the current visible screen.
      // The RingBuffer contains raw %output chunks with cursor-positioning
      // escapes that cause garbled content when replayed. capture-pane gives
      // a flat text snapshot that works correctly after term.reset() on the
      // client side. We capture the full scrollback (-S -) so the user
      // sees history, not just the visible viewport.
      let buffer = "";
      if (session.alive) {
