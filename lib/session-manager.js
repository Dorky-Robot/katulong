...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  // timers and holds the raw bytes between pushes. On flush it invokes
  // this callback with the concatenated payload, which we relay as an
  // `output` message directly — no intermediate RingBuffer, no cursor
  // plumbing. See lib/output-coalescer.js for the debounce semantics.
  //
  // Raptor 3: the Session push the exact bytes tmux emitted at the
  // current PTY dims. As long as clients are at the same dims when these
  // bytes arrive, the render is correct. Dimension transitions are
  // handled exclusively by the `snapshot` path below.
  const outputCoalescer = createOutputCoalescer({
    onFlush: (sessionName, data) => {
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      external,
      onData: (sessionName, payload) => {
        outputCoalescer.push(sessionName, payload);
      },
      onSnapshot: (snap) => {
        // Server-authoritative dimension transition. The coalescer has
        // already been flushed by the caller of resize() (see the
        // resizeClient / resizeSession / adopt paths that wrap each
        // resize in `sendSnapshotOnResize`), so old-dim bytes have
        // already reached old-dim clients. The snapshot carries the
        // new dims and the post-resize serialize so every subscribed
        // client can atomically transition.
        bridge.relay({
          type: "snapshot",
  ...
  return {
     ...
     *
     * Returns a snapshot `{ cols, rows, data, alive }` describing the
     * current ScreenState. The client applies `term.resize(cols, rows)
     * → term.clear → term.write(data)` atomically so its xterm starts
     * at the same dims as the server and shows identical content.
     *
     * `tracker.attach` may trigger a tmux resize if this client's dims
     * differ from the active one. That resize flows through the
     * Session._applyResize → onSnapshot path and is broadcast to all
     * OTHER clients of this session; this attaching client receives
     * the initial state via the return value below instead.
     *
     * @param {string} clientId
     * @param {string} sessionName
     * @param {number} cols
     * @param {number} rows
     * @returns {Promise<{ cols: number, rows: number, data: string, alive: boolean }>}
     */
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Flush pending output BEFORE letting the tracker trigger a resize.
      // The tracker's attach() may call session.resize, which emits a
      // snapshot to existing clients; without this flush, the old-dim
      // bytes would leak out AFTER the snapshot, arriving at already-
      // transitioned clients and producing garble.
      outputCoalescer.flush(name);

      tracker.attach(clientId, name, cols, rows);

      // Snapshot the current ScreenState and return it to the caller so
      // ws-manager can embed it in the `attached` response. This snapshot
      // reflects whatever state resulted from the tracker's resize (if
      // any), because _applyResize is awaited internally before the
      // snapshot broadcast.
     ...
     * Returns a snapshot the client can use to seed the xterm for this
     * session (separate pool entry from its primary attachment).
     *
     * Re-subscribes (carousel swipe back to a tile) still issue a fresh
     * snapshot under Raptor 3 — the cost is one full serialize + client
     * reset, and the benefit is that the xterm is guaranteed to match
     * the server's ScreenState. Under the old protocol we skipped the
     * snapshot to avoid mid-frame garble, but garble is impossible now:
     * the snapshot is the single source of truth.
     *
     * @param {string} clientId
     * @param {string} sessionName
     * @returns {Promise<{ cols: number, rows: number, data: string, alive: boolean, isNew: boolean }>}
     */
    async subscribeClient(clientId, sessionName) {
      ...
      // Flush queued output before snapshotting so the snapshot reflects
      // every byte that's about to be relayed to existing subscribers.
     ...
     *
     * Flushes any pending output BEFORE the tracker applies the resize
     * so old-dim bytes reach clients at old dims; the tracker then
     * invokes session.resize which (possibly after the idle-gate
     * deferral) calls _applyResize → onSnapshot, broadcasting the new
     * dims and serialized screen to all subscribed clients.
     *
     * @param {string} clientId
     * @param {number} cols
     * @param {number} rows
    ...
    resizeClient(clientId, cols, rows) {
      const sessionName = tracker.getSessionFor(clientId);
      if (sessionName) outputCoalescer.flush(sessionName);
     ...
     *
     * Same ordering invariant as resizeClient: flush old-dim bytes first
     * so they reach old-dim clients before the snapshot.
     *
     * @param {string} sessionName
     * @param {number} cols
     * @param {number} rows
