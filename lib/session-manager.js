...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  // Auto-cleanup timers for exited sessions.  When a shell exits, we delay
  // deletion by EXIT_CLEANUP_DELAY_MS so clients have time to display the
  // "[shell exited]" message before the session disappears from the list.
  const EXIT_CLEANUP_DELAY_MS = 2000;
  const exitCleanupTimers = new Map();  // sessionName -> Timer handle
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      onExit: (sessionName, exitCode) => {
        ...
        // Schedule auto-cleanup after a short delay so clients see the
        // "[shell exited]" message before the session disappears.
        const timer = setTimeout(() => {
          exitCleanupTimers.delete(sessionName);
          // Guard: session may have been manually deleted already
          if (!sessions.has(sessionName)) return;
          log.info("Auto-deleting exited session", { session: sessionName });
          deleteSession(sessionName);
        }, EXIT_CLEANUP_DELAY_MS);
        if (timer.unref) timer.unref();
        exitCleanupTimers.set(sessionName, timer);
      },
    });
  ...
  function deleteSession(name, { detachOnly = false } = {}) {
    ...
    // Cancel any pending exit-cleanup timer (manual delete pre-empts auto-cleanup)
    const exitTimer = exitCleanupTimers.get(name);
    if (exitTimer) { clearTimeout(exitTimer); exitCleanupTimers.delete(name); }
  ...
  return {
    ...
    shutdown() {
      ...
      for (const t of exitCleanupTimers.values()) clearTimeout(t);
      exitCleanupTimers.clear();
