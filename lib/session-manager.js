  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      if (session.alive) {
        ...
        // Force a single resize to send SIGWINCH so TUI apps redraw.
        // Bypasses the dedup guard — on reconnect we always want the app
        // to re-render at the (possibly unchanged) dimensions.
        session.forceResize(cols, rows);
      }
