...
export function createSessionManager({ bridge, shell, home }) {
  ...
  // Output coalescing: accumulate per-session output for one animation frame
  // (~16ms) before relaying, dramatically reducing WebSocket message count.
  const outputBuffers = new Map();   // sessionName -> accumulated string
  const outputOffsets = new Map();   // sessionName -> byte offset of first chunk in coalesce window
  const outputTimers = new Map();    // sessionName -> timeout handle
  const OUTPUT_COALESCE_MS = 16;

  function flushOutputBuffer(sessionName) {
    const timer = outputTimers.get(sessionName);
    if (timer) clearTimeout(timer);
    outputTimers.delete(sessionName);
    const data = outputBuffers.get(sessionName);
    const seq = outputOffsets.get(sessionName);
    outputBuffers.delete(sessionName);
    outputOffsets.delete(sessionName);
    if (data) bridge.relay({ type: "output", session: sessionName, data, seq });
  }
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      onData: (sessionName, data) => {
        const pending = outputBuffers.get(sessionName);
        if (pending !== undefined) {
          outputBuffers.set(sessionName, pending + data);
          // offset already recorded from first chunk in this coalesce window
        } else {
          ...
          outputOffsets.set(sessionName, session.outputBuffer.totalBytes - data.length);
          outputBuffers.set(sessionName, data);
          outputTimers.set(sessionName, setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_COALESCE_MS));
        }
      },
