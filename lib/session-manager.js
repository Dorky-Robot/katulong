...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Stateless: don't replay any buffer. The client resets the terminal,
      // and we send Ctrl-L to ask the running app to redraw itself.
      // This avoids all garbling issues from RingBuffer replay or capture-pane
      // width mismatches with TUI apps like Claude Code.
      if (session.alive) {
        session.write("\x0C"); // Ctrl-L — ask app to redraw
      }
