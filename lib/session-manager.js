...
/**
 * Per-client headless map for drift detection.
 *
 * Maps "clientId:sessionName" -> ClientHeadless instances so each client
 * gets a fingerprint computed at its own terminal dimensions. Shared across
 * tests and session-manager internals. PCH-2 will extend this into full
 * per-client rendering; for now it's scoped to drift detection.
 *
 * @returns {object} Map API with register, remove, removeClient, getBySession
 */
export function createClientHeadlessMap() {
  ...
  return {
    ...
    /**
     * Get a specific client's headless terminal for a session.
     * @param {string} clientId
     * @param {string} sessionName
     * @returns {ClientHeadless|null}
     */
    get(clientId, sessionName) {
      const key = `${clientId}:${sessionName}`;
      const entry = entries.get(key);
      return entry ? entry.headless : null;
    },
  };
}
...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  // Per-client headless map for drift detection (PCH-3).
  ...
  function scheduleIdleCheck(sessionName) {
    ...
    idleCheckTimers.set(sessionName, setTimeout(async () => {
      ...
      // Per-client drift detection (PCH-3): compute a fingerprint for each
  ...
  return {
    ...
    deleteSession(name, { detachOnly = false } = {}) {
      ...
      clientHeadlessMap.removeSession(name); // cleanup per-client headless (PCH-3)
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Register per-client headless for drift detection (PCH-3).
      // The headless replays from the session's RingBuffer at this client's
      // dimensions so fingerprints account for reflow differences.
      if (session.alive && cols && rows) {
        clientHeadlessMap.register(clientId, name, session.outputBuffer, cols, rows);
      }

      cancelNotification(name);
      // Resize BEFORE serialize so the snapshot matches the client's
      // dimensions. Without this, the serialized escape sequences
      // contain cursor positions for the old row count, which garble
      // TUI apps (Claude Code, vim, htop) when rendered at the new size.
      // The headless xterm resize is synchronous, so serializeScreen()
      // immediately sees the correct dimensions.
      if (session.alive && cols && rows) session.resize(cols, rows);
      const buffer = session.alive ? await session.serializeScreen() : "";
    ...
    detachClient(clientId) {
      ...
      clientHeadlessMap.removeClient(clientId); // cleanup per-client headless (PCH-3)
    },
    ...
    async subscribeClient(clientId, sessionName, cols, rows) {
      ...
      // Resize before serialize so snapshot matches client dimensions.
      if (!alreadySubscribed && session.alive && cols && rows) {
        session.resize(cols, rows);
      }
      const buffer = (!alreadySubscribed && session.alive) ? await session.serializeScreen() : "";
      const seq = session.outputBuffer.totalBytes;
     ...
     *
     * Updates the client's stored dimensions in the tracker (which handles
     * PTY resize for the active client), and also resizes the client's
     * ClientHeadless so it reflows at the new dimensions independently.
     *
     * @param {string} clientId
     * @param {number} cols
     * @param {number} rows
    ...
    resizeClient(clientId, cols, rows) {
      ...
      // Also resize this client's headless terminal (PCH-5).
      // Each client's headless tracks its own dimensions so xterm.js
      // reflows independently — non-active clients keep their size.
      const sessionName = tracker.getSessionFor(clientId);
      if (sessionName) {
        const headless = clientHeadlessMap.get(clientId, sessionName);
        if (headless) headless.resize(cols, rows);
      }
    },
    ...
    /**
     * Get a client's headless terminal for a session (PCH-5).
     * @param {string} clientId
     * @param {string} sessionName
     * @returns {ClientHeadless|null}
     */
