...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Stateless: no buffer replay. Return empty buffer and schedule a
      // resize toggle after a tick — the caller registers the client in
      // wsClients first, then the resize triggers SIGWINCH which makes
      // TUI apps (Claude Code, vim, etc.) redraw via live %output.
      if (session.alive) {
        const c = cols, r = rows;
        process.nextTick(() => {
          session.resize(Math.max(1, c - 1), r);
          setTimeout(() => session.resize(c, r), 50);
        });
      }
