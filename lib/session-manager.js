...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // scrollback snapshot and already in the RingBuffer.  By taking the
      // seq here, seq-init tells the client to start pulling AFTER the
      // captured state, preventing duplicate writes.
      cancelNotification(name);
      const seq = session.outputBuffer.totalBytes;
      // Capture cursor position so the client can sync xterm's cursor
      // with tmux's actual cursor (critical for cached tab switches).
