...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
     ...
     * @param {string} clientId
     * @param {string} sessionName
     * @returns {{ seq: number, alive: boolean }}
     */
    subscribeClient(clientId, sessionName) {
      if (!sessionName) throw new Error("Session name required");
      const session = sessions.get(sessionName);
      ...
      // Flush any pending output so the seq number is accurate
      flushOutputBuffer(sessionName);
