...
export function createSessionManager({ bridge, shell, home }) {
  ...
  // Per-session state grouped in a single Map to keep fields in sync.
  const outputCoalesce = new Map();  // sessionName -> { data, seq, timer }
  ...
  async function adoptSession(name, tmuxName, cols = DEFAULT_COLS, rows = DEFAULT_ROWS, { external = false } = {}) {
    ...
    const session = new Session(name, tmuxName, {
      ...
      onData: (sessionName, data) => {
        ...
        if (existing) {
          existing.data += data;
          // seq already recorded from first chunk in this coalesce window
        } else {
          ...
          outputCoalesce.set(sessionName, {
            ...
            timer: setTimeout(() => flushOutputBuffer(sessionName), OUTPUT_COALESCE_MS),
          });
        }
      },
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      let buffer = "";
      // Flush any pending coalesced output and snapshot the sequence position
      // BEFORE async captures.  This ensures seq-init is at a clean boundary —
      // any output arriving during the async captureScrollback window will
      // have seq >= this value and be delivered via the normal bridge path
      // once the client is registered in the routing table.
      flushOutputBuffer(name);
