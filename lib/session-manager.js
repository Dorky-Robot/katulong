...
import { TERMINAL_COLS, TERMINAL_ROWS_DEFAULT } from "./terminal-config.js";

const MAX_BUFFER_BYTES = 20 * 1024 * 1024; // 20 MB per session
const MAX_SESSIONS = 20;
const CHILD_COUNT_INTERVAL_MS = 5000;
const DEFAULT_COLS = TERMINAL_COLS;
...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  // Per-client headless terminals: "clientId:sessionName" -> ClientHeadless
  // Each client gets its own headless xterm at the client's dimensions so
  // serialization produces correctly reflowed output for that viewport.
  const clientHeadlessMap = new Map();

  function clientHeadlessKey(clientId, sessionName) {
    return `${clientId}:${sessionName}`;
  }

  /**
   * Create a per-client headless xterm for a client-session pair.
   * Replays the session's RingBuffer so the headless state is current.
   */
  function createClientHeadlessFor(clientId, sessionName, cols, rows) {
    const session = sessions.get(sessionName);
    if (!session) return null;
    const ch = new ClientHeadless(session.outputBuffer, cols, rows);
    clientHeadlessMap.set(clientHeadlessKey(clientId, sessionName), ch);
    return ch;
  }

  /**
   * Dispose and remove a per-client headless for a client-session pair.
   */
  function disposeClientHeadless(clientId, sessionName) {
    const key = clientHeadlessKey(clientId, sessionName);
    const ch = clientHeadlessMap.get(key);
    if (ch) {
      ch.dispose();
      clientHeadlessMap.delete(key);
    }
  }

  /**
   * Dispose all per-client headless instances for a given client.
   */
  function disposeAllClientHeadless(clientId) {
    const prefix = `${clientId}:`;
    for (const [key, ch] of clientHeadlessMap) {
  ...
  /**
   * Dispose all per-client headless instances for a given session.
   */
  function disposeAllHeadlessForSession(sessionName) {
  ...
  function notifyDataAvailable(sessionName, fromSeq) {
    if (notificationPending.has(sessionName)) return; // coalesce — fromSeq is from first chunk
    notificationPending.set(sessionName, setImmediate(() => {
      ...
      const data = session.outputBuffer.sliceFrom(fromSeq);
  ...
  return {
    ...
    deleteSession(name, { detachOnly = false } = {}) {
      ...
      // Dispose all per-client headless instances for this session
      disposeAllHeadlessForSession(name);
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Dispose any previous per-client headless for this client-session pair
      // (handles re-attach after switch, reconnect, etc.)
      disposeClientHeadless(clientId, name);

      let buffer = "";
      if (session.alive && cols && rows) {
        // Create per-client headless at client dimensions and serialize from it.
        // This avoids resizing the shared headless (which would garble other
        // clients viewing the same session at different dimensions).
        const ch = createClientHeadlessFor(clientId, name, cols, rows);
        if (ch) {
          buffer = await ch.serializeScreen();
        } else {
          // Fallback to shared headless (PCH-7 will remove this path)
    ...
    detachClient(clientId) {
      ...
      disposeAllClientHeadless(clientId);
    },
    ...
    async subscribeClient(clientId, sessionName, cols, rows) {
      ...
      if (!alreadySubscribed && session.alive && cols && rows) {
        // Create per-client headless at client dimensions for this subscription.
        disposeClientHeadless(clientId, sessionName);
        const ch = createClientHeadlessFor(clientId, sessionName, cols, rows);
        if (ch) {
          buffer = await ch.serializeScreen();
        } else {
          // Fallback to shared headless (PCH-7 will remove this path)
    ...
    getClientHeadless(clientId, sessionName) {
      return clientHeadlessMap.get(clientHeadlessKey(clientId, sessionName));
    },
    ...
    async restoreSessions() {
      ...
      // Resize any restored session that doesn't match DEFAULT_COLS.
      for (const [name, session] of sessions) {
    ...
    shutdown() {
      ...
      // Dispose all per-client headless instances
      for (const ch of clientHeadlessMap.values()) ch.dispose();
      clientHeadlessMap.clear();
