...
import {
  ...
  cleanTmuxServerEnv, tmuxListSessions, tmuxKillSession, tmuxListSessionsDetailed,
} from "./session.js";
...
export function createSessionManager({ bridge, shell, home }) {
   ...
   */
  function getActiveClientForSession(sessionName) {
    let activeId = null;
    let latestTime = 0;
   ...
   */
  function clientCountForSession(sessionName) {
    let count = 0;
  ...
  function markActive(clientId) {
    const info = clients.get(clientId);
    if (!info) return;
    const prevActive = getActiveClientForSession(info.session);
    info.lastActiveAt = Date.now();
    const nowActive = getActiveClientForSession(info.session);

    // If active client changed and the new active has stored dimensions,
    // apply their dimensions to tmux and notify other clients.
  ...
  return {
    ...
    async renameSession(oldName, newName) {
      ...
      // Also check for unmanaged tmux sessions with the same name
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      if (!session) {
        // Check for tmux name collision before spawning
        const newTmuxName = tmuxSessionName(name);
        for (const s of sessions.values()) {
      ...
      clients.set(clientId, { session: name, lastActiveAt: 0, cols, rows });
      // Attaching is an explicit user action — make this client active so its
      // dimensions are applied to tmux and broadcast to other viewers.
      // lastActiveAt starts at 0 so markActive() detects the active-client change.
      markActive(clientId);

      let buffer = "";
      if (session.alive) {
        const snapshot = await captureVisiblePane(session.tmuxName);
        if (snapshot) buffer = snapshot;

        // Always resize tmux to the attaching client's dimensions since
        // markActive() just made them the active client.
        session.resize(cols, rows);
        // Delay the SIGWINCH toggle so the snapshot has time to reach the
        // client and render before the TUI redraws over it.
        // Use nextTick for new sessions (no snapshot) for faster prompt delivery.
        const delay = snapshot ? 150 : 0;
        setTimeout(() => {
          const s = sessions.get(name);
          if (!s?.alive) return;
          s.resize(Math.max(1, cols - 1), rows);
          setTimeout(() => {
            const s2 = sessions.get(name);
            if (s2?.alive) s2.resize(cols, rows);
          }, RESIZE_TOGGLE_DELAY_MS);
        }, delay);
      }
    ...
    detachClient(clientId) {
      const info = clients.get(clientId);
      if (info) {
        const wasActive = getActiveClientForSession(info.session) === clientId;
        clients.delete(clientId);

        // If the active client disconnected, promote the next most-active client
        if (wasActive) {
          const newActiveId = getActiveClientForSession(info.session);
          if (newActiveId) {
    ...
    resizeClient(clientId, cols, rows) {
      ...
      // Always store this client's preferred dimensions
      info.cols = cols;
      info.rows = rows;

      const activeId = getActiveClientForSession(info.session);
      if (activeId === clientId || clientCountForSession(info.session) <= 1) {
        // Active client (or only client) — apply to tmux
        aliveSessionFor(clientId)?.resize(cols, rows);
        // Broadcast to other clients on this session
        if (clientCountForSession(info.session) > 1) {
          bridge.relay({
            type: "resize-sync",
            session: info.session,
            cols,
      ...
      // Non-active clients: dimensions stored but not applied to tmux
    },
