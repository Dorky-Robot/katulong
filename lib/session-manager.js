...
export function createSessionManager({ bridge, shell, home }) {
  ...
  return {
    ...
    async killTmuxSession(tmuxName) {
      if (!tmuxName || typeof tmuxName !== "string") return { error: "Invalid name" };
      if (tmuxName.length > 128 || /[:.\/]/.test(tmuxName)) return { error: "Invalid tmux session name" };
      if (isAlreadyManaged(tmuxName)) return { error: "Cannot kill managed session — use DELETE /sessions/:name instead" };
      // Refuse to kill sessions with external (non-control-mode) clients
      const externalSessions = await tmuxListSessionsDetailed();
      if (externalSessions.has(tmuxName)) return { error: "Session has attached clients — detach them first" };
      try {
        ...
        return { error: err.message || "Failed to kill tmux session" };
      }
    },
