...
export function createSessionManager({ bridge, shell, home, dataDir }) {
  ...
  const outputCoalesce = new Map();  // sessionName -> { fromSeq, idle, cap }
  function notifyDataAvailable(sessionName, fromSeq) {
    let pending = outputCoalesce.get(sessionName);
    if (pending) {
      // Reset idle timer — more output arriving
      clearTimeout(pending.idle);
      pending.idle = setTimeout(() => flushOutput(sessionName), 2);
      return;
    }
    pending = {
      fromSeq,
      idle: setTimeout(() => flushOutput(sessionName), 2),
      cap: setTimeout(() => flushOutput(sessionName), 16),
    };
    outputCoalesce.set(sessionName, pending);
  }

  function flushOutput(sessionName) {
    const pending = outputCoalesce.get(sessionName);
    if (!pending) return;
    clearTimeout(pending.idle);
  ...
  function countTmuxPaneProcesses(tmuxName) {
    return new Promise((resolve) => {
      execFile("tmux", [...tmuxSocketArgs(), "list-panes", "-t", tmuxName, "-F", "#{pane_pid}"], { timeout: 5000 }, (err, stdout) => {
  ...
  return {
    ...
    async attachClient(clientId, sessionName, cols, rows) {
      ...
      // Serialize from the shared session headless. The shared headless is
      // written live at the current PTY dims and reflowed on resize via
      // xterm.js, so it always reflects truth. See PCH-7 — per-client
      // replay of RingBuffer history at client-specific dims could not
      // correctly re-interpret absolute cursor escapes.
    ...
    async subscribeClient(clientId, sessionName, cols, rows) {
      ...
      // Flush queued output first so existing subscribers see the bytes
      // that landed in the coalescer before this subscribe — cancelling
      // (the prior behavior) silently dropped them.
      flushOutput(sessionName);

      // Serialize from the shared session headless on first subscribe.
      // PCH-7 removed per-client headless — see attachClient for the full
      // rationale.
      let buffer = "";
      if (!alreadySubscribed && session.alive) {
    ...
    shutdown() {
      ...
      for (const p of outputCoalesce.values()) { clearTimeout(p.idle); clearTimeout(p.cap); }
