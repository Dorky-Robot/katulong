...
export function createDispatchRoutes(ctx) {
  ...
  function finishRefineSession(session, outcome) {
    session.status = outcome; // "completed" | "failed"
    // The failed path deliberately does NOT include a detail string on the
    // wire: the underlying error from the claude subprocess can carry up to
    // 500 bytes of stderr, which may include internal paths or other
    // subprocess noise. The full error is logged server-side by the caller.
    const detail = outcome === "failed" ? "Refinement failed — check server logs" : null;
    ...
    // Accepts { featureIds: [...] } or { all: true } to refine all raw features.
    {
      method: "POST", path: "/api/dispatch/refine", handler: auth(csrf(async (req, res) => {
        ...
        refiner.refineBatch(store, featureIds, {
          ...
          finishRefineSession(session, "failed");
        });
      })),
    },
