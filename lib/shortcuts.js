...
/**
 * Result types for shortcuts operations
 */

export class ShortcutsSuccess {
 ...
 * @returns {ShortcutsSuccess|ShortcutsFailure}
 */
export function parseShortcuts(jsonText) {
  try {
    ...
    return new ShortcutsSuccess(shortcuts);
  } catch (err) {
    return new ShortcutsFailure("parse-error", err.message);
  }
}
...
export function loadShortcuts(filePath) {
  try {
    if (!existsSync(filePath)) {
      return new ShortcutsSuccess([]);
    }

    const data = readFileSync(filePath, "utf-8");
    return parseShortcuts(data);
  } catch (err) {
    return new ShortcutsFailure("file-error", err.message);
  }
}
