...
/**
 * Legacy wrapper — decodes per-line (can produce replacement characters
 * if a multi-byte character spans %output boundaries).
 * Prefer unescapeTmuxOutputBytes + StringDecoder for production use.
 *
 * If the input ends with a partial octal escape (e.g. `…\34`), this
 * wrapper passes those trailing chars through as literal bytes, matching
 * the pre-carry behavior. Production callers should use
 * unescapeTmuxOutputBytes directly so they can preserve the carry across
 * %output boundaries.
 */
export function unescapeTmuxOutput(s) {
  const { bytes, carry } = unescapeTmuxOutputBytes(s);
  if (!carry) return bytes.toString("utf-8");
  // Encode the carry's chars as literal UTF-8 bytes (each carry char is
  // ASCII: '\', '0'-'7') and append.
  const trail = Buffer.from(carry, "utf-8");
  return Buffer.concat([bytes, trail]).toString("utf-8");
}
