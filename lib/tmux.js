...
/**
 * Capture visible pane content plus cursor/mode metadata for resync.
 * Returns { data, cursorX, cursorY, alternateScreen } or null on failure.
 */
export async function capturePaneWithMetadata(name) {
  const [pane, meta] = await Promise.all([
    captureVisiblePane(name),
    tmuxExec(["display-message", "-t", name, "-p", "#{cursor_x},#{cursor_y},#{alternate_on}"]),
  ]);
  if (!pane) return null;
  let cursorX = 0, cursorY = 0, alternateScreen = false;
  ...
  return { data: pane, cursorX, cursorY, alternateScreen };
}
