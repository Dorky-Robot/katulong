 ...
 * Returns a Buffer of raw bytes — caller must handle UTF-8 decoding
 * (use a persistent StringDecoder to handle multi-byte characters
 * split across %output boundaries).
 */
export function unescapeTmuxOutputBytes(s) {
  const bytes = [];
  let i = 0;
  while (i < s.length) {
    if (s[i] === "\\" && i + 3 < s.length) {
...
export function unescapeTmuxOutput(s) {
  return unescapeTmuxOutputBytes(s).toString("utf-8");
}
