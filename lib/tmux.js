...
/**
 * Return tmux socket args (`-L <name>`) when KATULONG_TMUX_SOCKET is set,
 * or an empty array otherwise.
 *
 * Used by the e2e test harness to isolate test tmux sessions on a dedicated
 * tmux server, so smoke/session test leftovers never land on the developer's
 * default socket (where tmux-continuum would snapshot and restore them on
 * reboot — polluting their real sessions forever).
 *
 * The name must be a safe filename (alphanumeric + `_`/`-`); tmux joins it
 * with `/tmp/tmux-$UID/` to form the socket path. Anything else is rejected
 * to keep this out of reach of shell metacharacters and path traversal.
 *
 * NOTE: this is deliberately NOT a way to disable tmux isolation for auth
 * purposes — it only changes *which* tmux server katulong talks to.
 */
export function tmuxSocketArgs() {
  const socket = process.env.KATULONG_TMUX_SOCKET;
  if (!socket) return [];
  if (!/^[A-Za-z0-9_-]+$/.test(socket)) {
    throw new Error(
      `KATULONG_TMUX_SOCKET must match /^[A-Za-z0-9_-]+$/, got: ${JSON.stringify(socket)}`
    );
  }
  return ["-L", socket];
}
 ...
 *
 * Automatically prepends KATULONG_TMUX_SOCKET flags when set — all call
 * sites that go through this helper are socket-isolated for free.
 *
 * Declared `async` so that a synchronous throw from `tmuxSocketArgs()`
 * (invalid env var) surfaces as a rejected Promise rather than an
 * exception that bubbles out of `tmuxExec` synchronously. All callers
 * `await` this function, so a rejected Promise is the only shape that
 * composes correctly with their error handling.
 */
export async function tmuxExec(args) {
  const fullArgs = [...tmuxSocketArgs(), ...args];
  return new Promise((resolve) => {
    execFile("tmux", fullArgs, { timeout: 10000 }, (err, stdout, stderr) => {
...
export async function tmuxNewSession(name, cols, rows, shell, env, cwd) {
  ...
  // shell is operator config (envConfig.shell), not request input, but
  // quote it anyway — a SHELL path containing spaces or metacharacters
  // would otherwise be interpreted by /bin/sh.
  const shellPart = `'${shell.replace(/'/g, "'\\''")}'`;
  const shellCmd = `${envParts.join(" ")} cd ${cwdPart} 2>/dev/null; exec ${shellPart} -l`;
  ...
  // The shared option set lives in applyTmuxSessionOptions so spawned and
  // adopted sessions can't drift; only the new-session extras stay here.
  await Promise.all([
    applyTmuxSessionOptions(name),
    tmuxExec(["set-option", "-t", name, "history-limit", "50000"]),
    // unbind-key has no -t flag; these are global and idempotent —
    // intentionally not part of applyTmuxSessionOptions.
    tmuxExec(["unbind-key", "\""]),
    tmuxExec(["unbind-key", "%"]),
  ]);
}
...
/**
 * Capture the pane's visible screen plus N lines of scrollback as plain
 * text. No ANSI escapes — this is meant for inspection surfaces
 * (summarizer prompts, log collection) that would strip them anyway.
 *
 * `lines` maps to tmux's `-S -<lines>` start-position. Pass a reasonable
 * upper bound: tmux silently clamps to the pane's history limit.
 */
export async function captureRecentPane(name, lines = 500) {
  const { code, stdout } = await tmuxExec([
    "capture-pane", "-t", name, "-p", "-J", "-S", `-${Math.max(1, lines | 0)}`,
  ]);
