...
import { stat, readdir, mkdir, rename, cp, rm, writeFile, readFile } from "node:fs/promises";
  ...
  return [
    ...
    // --- Write file (save from editor) ---
    {
      method: "POST", path: "/api/files/write", handler: auth(csrf(async (req, res) => {
        ...
        // Must be an existing regular file — no creating new files via this endpoint
        let stats;
        try {
          stats = await stat(resolved);
        } catch {
