...
async function resolveMdnsCandidate(candidateObj) {
  ...
  if (!c || !c.includes(".local")) return candidateObj;

  const fields = c.split(/\s+/);
  // candidate:<fn> <comp> <proto> <pri> <IP> <port> typ host
  const addrIndex = 4;
  const addr = fields[addrIndex];
  if (!addr || !addr.endsWith(".local")) return candidateObj;

  try {
    const { address } = await lookup(addr);
    log.info("P2P resolved mDNS candidate", { mdns: addr, resolved: address });
    fields[addrIndex] = address;
