 ...
 *
 * Remote access is handled by an external tunnel (ngrok, Cloudflare Tunnel, etc.)
 * which terminates TLS and forwards requests to localhost. Tunnel traffic arrives
 * on a loopback socket but with a non-localhost Host header, so it is correctly
 ...
 * - "internet" - Remote access via tunnel, requires passkey
 *
 * @param {import('http').IncomingMessage} req - HTTP request
