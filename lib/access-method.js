...
/**
 * Loopback addresses: IPv4, IPv6, and IPv4-mapped IPv6.
 * Used to detect whether a socket connection originates from the same machine.
 *
 * NOTE: A loopback socket address alone does NOT mean the request is local —
 * tunnel services (ngrok, Cloudflare Tunnel) forward traffic from loopback.
 * Always check Host/Origin headers in addition to socket address.
 *
 * @param {string} addr - Socket remote address
 * @returns {boolean} True if address is a loopback address
 */
...
/**
 * Hostname suffixes for known HTTPS-only tunnel services.
 * These services always terminate TLS at their edge and forward to localhost via HTTP.
 * When the Host header matches one of these suffixes, the connection is treated as HTTPS.
 */
export const TUNNEL_HOSTNAMES = [
  ".ngrok.app",
  ".ngrok.io",
  ".trycloudflare.com",
  ".loca.lt",
