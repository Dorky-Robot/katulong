...
export async function reconcilePaneGit(session, paneCwd, opts = {}) {
  ...
  if (typeof paneCwd !== "string" || !paneCwd) {
    const existingPane = session.meta?.pane ?? null;
    if (!existingPane?.git) return false;
    try {
      session.setMeta("pane", { ...existingPane, git: null });
      return true;
    } catch (err) {
      log.warn("Failed to clear meta.pane.git", {
        session: session.name, cwd: paneCwd, error: err.message,
      });
      return false;
    }
  }
  ...
  // Session may have been killed during the git probe (up to 2s). Don't
  // write back to a dead session — mirrors the pattern in
  // session-summarizer.js which also has a post-await liveness guard.
  if (!session.alive) return false;

  // Re-read `meta.pane` AFTER the await — not before. Snapshotting
  // pre-await would clobber any concurrent writer that updated `cwd`
  // (or any future field) while git was probing. The per-session
  // inFlight guard in `startChildCountMonitor` makes this race
  // unreachable from the monitor today, but reading fresh here keeps
  // the function correct for any external caller.
  const existingPane = session.meta?.pane ?? null;
