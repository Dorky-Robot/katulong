...
import { readBody } from "../request-util.js";
...
const deviceAuthRequests = new Map(); // requestId -> { code, userAgent, createdAt, approved, consumed, sessionToken, credentialId }
...
// Expired entries were previously only deleted when their specific
// requestId was polled — requests that were created but never polled
// (abandoned devices, probing) accumulated for the life of the process.
// Module scope (not inside createAuthRoutes) because deviceAuthRequests
// is module state; unref'd so it never holds the process open.
const deviceAuthSweepTimer = setInterval(() => {
  for (const [id, request] of deviceAuthRequests) {
    if (isDeviceAuthExpired(request)) deviceAuthRequests.delete(id);
  }
}, DEVICE_AUTH_TTL_MS);
if (deviceAuthSweepTimer.unref) deviceAuthSweepTimer.unref();
...
export function createAuthRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/auth/login/options", rateLimit: true, handler: async (req, res) => {
      // No body expected; drain with a small cap so oversized payloads to
      // this public endpoint are rejected instead of buffered.
    ...
    { method: "POST", path: "/auth/logout", handler: csrf(async (req, res) => {
      ...
      if (token) {
        ...
        if (result.state) {
          log.info("Session logged out", { ip: req.socket?.remoteAddress });
        }
      }
    ...
    { method: "POST", path: "/auth/device-auth/request", rateLimit: true, handler: async (req, res) => {
      // No body expected; drain with a small cap (public endpoint).
      await readBody(req, 1024);
      ...
      log.info("Device auth request created", { requestId, ip: req.socket?.remoteAddress });
    ...
    { method: "GET", path: "/auth/device-auth/status", rateLimit: true, handler: async (req, res) => {
      ...
      // Ergonomic guard, NOT a security boundary: the User-Agent is
      // broadcast alongside the requestId, so anyone who saw one saw the
      // other. The real issuance gate is the code check on /approve. This
      // just keeps stray polls (other tabs, monitoring) from racing the
      // requesting browser. Report "expired" without deleting so the real
      // device keeps working.
      const pollerUA = req.headers["user-agent"] || "Unknown";
      if (pollerUA !== request.userAgent) {
        return json(res, 200, { status: "expired" });
      }
      ...
      // Single-issuance gate: two concurrent polls both passing the
      // approved check would each mint a login token. The flag is set
      // synchronously before the first await, so only one racer enters.
      if (request.consumed) {
      ...
      let result;
      try {
        result = await withStateLock((currentState) => {
          if (!currentState) return {};
        ...
        // A throw inside the lock must not leave the gate stuck closed —
        // the device would be permanently approved-but-sessionless.
        request.consumed = false;
        throw err;
      }

      if (!result.state) {
        // Auth state unavailable (unreadable/corrupt). Keep the request
        // and clear the gate so the device can retry — deleting here
        // would strand an approved device with no recovery path.
        request.consumed = false;
        return json(res, 503, { error: "Auth state unavailable, retry" });
      }
    ...
    { method: "POST", path: "/auth/device-auth/approve", rateLimit: true, handler: auth(csrf(async (req, res) => {
      const { requestId, code } = await parseJSON(req);
      ...
      // The code is the approval secret (it's deliberately kept out of
      // server logs, unlike the requestId). Enforce it server-side —
      // approval must come through the UI that received the broadcast,
      // not from anything that merely learned a requestId.
      const codeBuf = Buffer.from(code);
      const expectedBuf = Buffer.from(request.code);
      if (codeBuf.length !== expectedBuf.length || !timingSafeEqual(codeBuf, expectedBuf)) {
        log.warn("Device auth approve rejected: code mismatch", { requestId });
        return json(res, 403, { error: "Code mismatch" });
      }
    ...
    { method: "POST", path: "/auth/device-auth/deny", rateLimit: true, handler: auth(csrf(async (req, res) => {
