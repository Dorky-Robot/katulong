...
export function createAuthRoutes(ctx) {
  ...
  return [
    ...
    { method: "GET", path: "/auth/device-auth/status", rateLimit: true, handler: async (req, res) => {
      ...
      // The requestId is broadcast to every authenticated WS client (the
      // approval UI needs it), so any of those clients could poll an
      // approved request and mint themselves an extra session. Require
      // the poller's User-Agent to match the device that created the
      // request — weak binding, but it must come from the same browser.
      // Report "expired" without deleting so the real device keeps working.
