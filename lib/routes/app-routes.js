...
import {
  ...
  unlinkSync, statSync, readdirSync, linkSync,
} from "node:fs";
...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    // --- Notes (flat-name markdown files) ---
    //
    // Identity model: name = filename = the only handle on the note. The
    // tile renderer keeps tile title in lockstep with the underlying file
    // via PATCH; there is no session coupling. Files live as
    // DATA_DIR/notes/<name>.md (raw name, not URL-encoded) so operators can
    // grep/cat/vim them with standard tools.
    ...notesRoutes({ json, parseJSON, auth, csrf, callOllama, DATA_DIR }),
  ];
}

// Filenames the user types must round-trip through HTTP path segments and
// the local filesystem cleanly. Reject path traversal, hidden files, OS
// reserved separators, and trailing dots/spaces (Windows-hostile, but also
// confusing on macOS Finder). 1..120 chars keeps us inside common path
// length budgets even when nested in long DATA_DIR paths.
const NOTE_NAME_RE = /^[A-Za-z0-9 _.\-()[\]]{1,120}$/;
function isValidNoteName(name) {
  if (typeof name !== "string") return false;
  if (!NOTE_NAME_RE.test(name)) return false;
...
function listNotes(notesDir) {
  ...
  for (const e of entries) {
    ...
    if (!isValidNoteName(name)) continue;
    try {
      const st = statSync(join(notesDir, e.name));
...
function nextUntitledName(notesDir) {
  const taken = new Set(listNotes(notesDir).map(n => n.name));
  let n = 1;
  while (taken.has(`untitled-${n}`)) n++;
  return `untitled-${n}`;
}

function notePathFor(notesDir, name) {
  if (!isValidNoteName(name)) return null;
  const filePath = normalize(join(notesDir, name + ".md"));
  if (!filePath.startsWith(notesDir + "/") && filePath !== join(notesDir, name + ".md")) return null;
  return filePath;
}

// System prompt for auto-naming. Asks the model for one short slug — no
// preamble, no quotes — so we can use the entire response verbatim after
// minimal sanitization. Constraint set is tight on purpose: the result
// becomes a filesystem path segment that must satisfy isValidNoteName.
const AUTO_NAME_SYSTEM_PROMPT = `You name a note based on its content. Output ONE short slug, 2 to 5 words, lowercase, words separated by spaces or hyphens. No quotes, no punctuation other than hyphens, no preamble, no explanation, no trailing notes. The slug must be a valid filename without an extension. If the content is empty or you cannot summarize, output exactly: untitled.`;

function sanitizeSuggestedName(raw) {
  if (typeof raw !== "string") return null;
  let s = raw.trim().split(/\r?\n/)[0].trim();
  // Strip wrapping quotes the model sometimes adds despite the prompt.
  s = s.replace(/^["'`]+|["'`]+$/g, "").trim();
  // Drop anything that is not in the allowed set.
  s = s.replace(/[^A-Za-z0-9 _.\-()[\]]/g, "");
  s = s.replace(/\s+/g, " ").trim();
  if (!s) return null;
  if (s.length > 60) s = s.slice(0, 60).trim();
  if (!isValidNoteName(s)) return null;
  return s;
}

function notesRoutes({ json, parseJSON, auth, csrf, callOllama, DATA_DIR }) {
  const notesDirOf = () => join(DATA_DIR, "notes");

  return [
    { method: "GET", path: "/api/notes", handler: auth((req, res) => {
      try {
        json(res, 200, { notes: listNotes(notesDirOf()) });
      } catch (err) {
        log.error("Failed to list notes", { error: err.message });
        json(res, 500, { error: "Failed to list notes" });
      }
    })},

    // Auto-naming via the shared ollama cascade. Stateless: the renderer
    // hands in the content, gets back a suggestion, and decides whether
    // to PATCH the file rename. Returns { suggestion: null } on any
    // backend failure so the caller can retry later without surfacing an
    // error to the user — auto-naming is a nice-to-have, not load-bearing.
    { method: "POST", path: "/api/notes/auto-name", handler: auth(csrf(async (req, res) => {
      const body = await parseJSON(req).catch(() => ({}));
      const content = body && typeof body.content === "string" ? body.content : "";
      if (!content.trim()) return json(res, 200, { suggestion: null });
      if (typeof callOllama !== "function") return json(res, 200, { suggestion: null });
      try {
        const excerpt = content.slice(0, 2000);
        const raw = await callOllama(excerpt, { systemPrompt: AUTO_NAME_SYSTEM_PROMPT });
        const suggestion = sanitizeSuggestedName(raw);
        json(res, 200, { suggestion });
      } catch (err) {
        log.error("Auto-name failed", { error: err.message });
        json(res, 200, { suggestion: null });
      }
    }))},

    { method: "POST", path: "/api/notes", handler: auth(csrf(async (req, res) => {
      const body = await parseJSON(req).catch(() => ({}));
      const notesDir = notesDirOf();
      mkdirSync(notesDir, { recursive: true });

      let name = body && typeof body.name === "string" ? body.name : null;
      if (name === null) {
        name = nextUntitledName(notesDir);
      } else if (!isValidNoteName(name)) {
        return json(res, 400, { error: "Invalid name" });
      }
      const filePath = notePathFor(notesDir, name);
      if (!filePath) return json(res, 400, { error: "Invalid name" });

      const content = body && typeof body.content === "string" ? body.content : "";
      try {
        // `wx` = O_CREAT | O_EXCL: atomic create-only-if-absent. Closes
        // the TOCTOU window between an existsSync check and the write,
        // which two concurrent POSTs (e.g. duplicate untitled-1 picks)
        // would otherwise race through.
        writeFileSync(filePath, content, { encoding: "utf-8", flag: "wx" });
        const st = statSync(filePath);
        json(res, 201, { name, mtime: st.mtimeMs, size: st.size });
      } catch (err) {
        if (err.code === "EEXIST") {
          return json(res, 409, { error: "Note already exists", name });
        }
        log.error("Failed to create note", { name, error: err.message });
        json(res, 500, { error: "Failed to create note" });
      }
    }))},
    ...
    { method: "GET", prefix: "/api/notes/", handler: auth((req, res, name) => {
      ...
      if (!filePath) return json(res, 400, { error: "Invalid name" });
      try {
        if (!existsSync(filePath)) return json(res, 404, { error: "Note not found" });
    ...
    { method: "PUT", prefix: "/api/notes/", handler: auth(csrf(async (req, res, name) => {
      ...
      if (!filePath) return json(res, 400, { error: "Invalid name" });
      const body = await parseJSON(req);
      if (!body || typeof body.content !== "string") return json(res, 400, { error: "Invalid content" });
      try {
        mkdirSync(notesDir, { recursive: true });
    ...
    { method: "PATCH", prefix: "/api/notes/", handler: auth(csrf(async (req, res, name) => {
      const notesDir = notesDirOf();
      const oldPath = notePathFor(notesDir, name);
      if (!oldPath) return json(res, 400, { error: "Invalid name" });
      const body = await parseJSON(req).catch(() => ({}));
      const newName = body && typeof body.newName === "string" ? body.newName : null;
      if (!newName || !isValidNoteName(newName)) return json(res, 400, { error: "Invalid newName" });
      const newPath = notePathFor(notesDir, newName);
      if (!newPath) return json(res, 400, { error: "Invalid newName" });
      if (newName === name) return json(res, 200, { name });
      try {
        if (!existsSync(oldPath)) return json(res, 404, { error: "Note not found" });
        // POSIX link() fails with EEXIST if the destination exists; pair
        // it with unlink() of the source for an atomic move that cannot
        // clobber an existing target. renameSync's "destination wins"
        // semantics would let two concurrent renames silently overwrite
        // a third file's content via the same target name.
        try {
          linkSync(oldPath, newPath);
        } catch (e) {
          if (e.code === "EEXIST") return json(res, 409, { error: "Target name already exists" });
          throw e;
        }
        unlinkSync(oldPath);
        json(res, 200, { name: newName });
      } catch (err) {
        log.error("Failed to rename note", { name, newName, error: err.message });
        json(res, 500, { error: "Failed to rename note" });
      }
    }))},
