...
export function applyClaudeMetaFromHook(payload, sessionManager, logger = null) {
  ...
  if (event === "SessionEnd" || event === "Stop") {
    ...
    // Running/detectedAt are owned by the pane monitor and correctly
    // reflect whether Claude is live.
    return null;
  }
...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/api/claude-events", handler: auth(csrf(async (req, res) => {
      ...
      // Trace logging — helps diagnose the "sparkle opens wrong feed" class
      // of bug. Shows every hook hit (event + pane + session_id + verdict),
      // without spamming since Claude Code fires a handful per session.
      log?.info?.("claude-hook received", {
        event: payload.hook_event_name,
        pane: payload._tmuxPane,
