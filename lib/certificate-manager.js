...
export class CertificateManager {
  constructor(dataDir, instanceNameOrConfig) {
    ...
    this.backupsDir = join(this.tlsDir, "backups");
  ...
  async regenerateCA() {
    ...
    const keyTmp = join(this.tlsDir, "ca.key.tmp");

    writeFileSync(certTmp, certPem);
    writeFileSync(keyTmp, keyPem, { mode: 0o600 });

    renameSync(certTmp, caCertPath);
    renameSync(keyTmp, caKeyPath);
    ...
    log.info("CA certificate regenerated", { backupId });
   ...
   * @returns {string} Backup ID (timestamp)
   */
  async backupCA() {
    ...
    // Parse to get instance name
    const cert = forge.pki.certificateFromPem(caCert);
    const cnAttr = cert.subject.attributes.find(attr => attr.name === "commonName");
    const instanceName = cnAttr ? cnAttr.value.replace(" Local CA", "") : "Unknown";

    // Write backup with metadata
    const backupCertPath = join(this.backupsDir, `ca.crt.${backupId}`);
    const backupKeyPath = join(this.backupsDir, `ca.key.${backupId}`);
    const backupMetaPath = join(this.backupsDir, `ca.meta.${backupId}.json`);

    writeFileSync(backupCertPath, caCert);
    writeFileSync(backupKeyPath, caKey, { mode: 0o600 });
    writeFileSync(backupMetaPath, JSON.stringify({
      id: backupId,
      timestamp,
      instanceName,
    ...
    log.info("CA certificate backed up", { backupId, instanceName });

    return backupId;
  }

  /**
   * List available CA backups
  ...
  async listCABackups() {
    ...
    const metaFiles = files.filter(f => f.startsWith("ca.meta.") && f.endsWith(".json"));

    const backups = metaFiles.map(f => {
      const metaPath = join(this.backupsDir, f);
      try {
   ...
   */
  async restoreCABackup(backupId) {
    const backupCertPath = join(this.backupsDir, `ca.crt.${backupId}`);
    const backupKeyPath = join(this.backupsDir, `ca.key.${backupId}`);

    if (!existsSync(backupCertPath) || !existsSync(backupKeyPath)) {
      throw new Error(`Backup ${backupId} not found`);
    }

    log.info("Restoring CA certificate from backup", { backupId });

    // Read backup
    const caCert = readFileSync(backupCertPath, "utf-8");
    const caKey = readFileSync(backupKeyPath, "utf-8");

    // Write to current CA location atomically
    const caCertPath = join(this.tlsDir, "ca.crt");
    const caKeyPath = join(this.tlsDir, "ca.key");
    const certTmp = join(this.tlsDir, "ca.crt.tmp");
    const keyTmp = join(this.tlsDir, "ca.key.tmp");

    writeFileSync(certTmp, caCert);
    writeFileSync(keyTmp, caKey, { mode: 0o600 });
    ...
    log.info("CA certificate restored", { backupId });
  }

  /**
   * Clean up old CA backups, keeping the most recent N backups
   * @param {number} keepCount - Number of backups to keep
   */
  async cleanupOldBackups(keepCount = 5) {
    const backups = await this.listCABackups();
    ...
    for (const backup of toDelete) {
      const backupCertPath = join(this.backupsDir, `ca.crt.${backup.id}`);
      const backupKeyPath = join(this.backupsDir, `ca.key.${backup.id}`);
      const backupMetaPath = join(this.backupsDir, `ca.meta.${backup.id}.json`);

      try {
        if (existsSync(backupCertPath)) rmSync(backupCertPath);
        if (existsSync(backupKeyPath)) rmSync(backupKeyPath);
        if (existsSync(backupMetaPath)) rmSync(backupMetaPath);
      } catch (error) {
        log.warn("Failed to delete old backup", { backupId: backup.id, error: error.message });
      }
    }

    log.info("Cleaned up old CA backups", { deleted: toDelete.length, kept: keepCount });
  }
}
