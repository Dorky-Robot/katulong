 ...
 *
 * Originally also shared with a now-extracted `bridges/_lib/launchd-template.js`
 * — that bridge framework lives in its own repo (`ollama-bridge`) as of the
 * connect-ollama-bridge migration. The path list and XML escaper stay here
 * because anything else under `lib/cli/` that grows into a launchd-aware
 * subcommand should reuse the same well-tested primitives.
 */
