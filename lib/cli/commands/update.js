...
export default async function update(args) {
  ...
  if (method === "homebrew") {
    // After brew upgrade, THIS process is the OLD binary — ROOT points at a
    // deleted Cellar path.  We can't use in-process restart/start because
    // they'd spawn server.js from that dead path.  Instead: stop the old
    // server, then shell out to the NEW `katulong` binary (resolved via PATH,
    // which brew already relinked).
    if (existsSync(PLIST_PATH)) {
      console.log("\nRestarting via LaunchAgent...");
      try { exec("katulong service restart"); } catch { /* best effort */ }
    } else if (serverWasRunning) {
      console.log("\nRestarting server...");
      // Kill old server (still running from deleted Cellar path)
      const old = isServerRunning();
      if (old.running && old.pid) {
        try { process.kill(old.pid, "SIGTERM"); } catch { /* already gone */ }
        // Wait briefly for port to free
        const deadline = Date.now() + 5000;
        while (Date.now() < deadline && isServerRunning().running) {
          await new Promise(r => setTimeout(r, 200));
        }
      }
      // Start via the NEW binary on PATH
      try { exec("katulong start"); } catch { /* best effort */ }
    } else {
  ...
  // Non-Homebrew: restart in-process (ROOT is still valid)
