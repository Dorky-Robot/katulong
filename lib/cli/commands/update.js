...
export default async function update(args) {
  ...
  try {
    ...
    // Without the sentinel, brew post_install runs its own restart on top
    // of our _finish-upgrade restart — a double-restart race. Warn so the
    // operator can connect the dots if the upgrade behaves oddly.
    console.log(`  (could not write upgrade sentinel: ${err.message})\n`);
  }
    ...
    switch (method) {
      case "homebrew":
        try {
          ...
          // Continue with possibly-stale formula data, but say so —
          // otherwise "already up to date" below could be silently wrong.
          console.log("  (brew update failed; version check may be stale)\n");
        }
        try {
