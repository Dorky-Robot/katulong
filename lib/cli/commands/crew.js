...
async function output(args) {
  ...
  // Follow mode: poll with fromSeq
  let seq = result.seq;
  const poll = async () => {
    while (true) {
      await new Promise(r => setTimeout(r, 1000));
      try {
        const update = await api.get(`/sessions/${encodeURIComponent(sessionName)}/output?fromSeq=${seq}`);
        if (update.data) {
          process.stdout.write(update.data);
          seq = update.seq;
        } else if (update.screen) {
          // Buffer evicted — show snapshot and reset
          process.stdout.write(update.screen);
          seq = update.seq;
        }
      } catch (err) {
...
const subcommands = { list, status, spawn, exec, output, kill };
