...
export function buildAndWritePlist() {
  // Unlink first to avoid EPERM when the file is owned by launchd
  try {
...
async function restart() {
  ...
  // Without this, `katulong start --foreground` (run by launchd) sees the
  // old process still alive, prints "already running", exits 0, and launchd
  // considers the job done — leaving the stale process running.
  if (oldPid) {
  ...
  // (after brew upgrade, the old plist may reference the previous Cellar path)
  buildAndWritePlist();
