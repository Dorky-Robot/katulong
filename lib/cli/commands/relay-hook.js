...
// Resolve the current tmux pane id. Prefer a live `tmux display-message`
// query over `process.env.TMUX_PANE`: the env var is set when the shell
// starts and goes stale if the shell was re-parented, the user ran
// `tmux switch-client`, or the variable was exported from a sibling pane.
// Querying tmux through $TMUX is authoritative for the pane the hook is
// actually firing from.
function resolveTmuxPane() {
  ...
  try {
    ...
    // tmux missing or unreachable — fall through
  }
  const envPane = process.env.TMUX_PANE;
  return envPane && /^%\d+$/.test(envPane) ? envPane : null;
}
