...
async function create(args) {
  ...
  Expires: ${formatExpiry(data.expiresAt)}\n`);
    ...
    // Try to get external URL for a scannable pairing link
    let qrContent = data.token;
    try {
      const ext = await api.get("/api/external-url");
      if (ext.url) {
        qrContent = `${ext.url}/login?setup_token=${encodeURIComponent(data.token)}`;
        console.log(`  Pair: ${qrContent}\n`);
      }
    } catch { /* no external URL available */ }
