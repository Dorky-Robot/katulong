...
import { formatTable, formatExpiry } from "../format.js";
...
Subcommands:
  ...
  create <name> --qr <url>  Create + show QR code for pairing
  list                   List setup tokens
  revoke <id>            Revoke a token (and linked credential)

Options:
  --json          Output as JSON
  --qr <url>     Show QR code with link (e.g., --qr https://my.tunnel.com)
`);
}

async function create(args) {
  const jsonMode = args.includes("--json");
  const qrIdx = args.indexOf("--qr");
  const qrUrl = qrIdx >= 0 ? args[qrIdx + 1] : null;
  const filteredArgs = args.filter((a, i) => a !== "--json" && a !== "--qr" && (qrIdx < 0 || i !== qrIdx + 1));
  const name = filteredArgs.join(" ").trim();
  if (!name) {
    console.error("Usage: katulong token create <name> [--qr <url>]");
    process.exit(1);
  }
...
Save this token — it will not be shown again.`);
    ...
    if (qrUrl) {
      const url = `${qrUrl.replace(/\/$/, "")}/login?setup_token=${encodeURIComponent(data.token)}`;
      console.log(`\nScan to pair:\n`);
