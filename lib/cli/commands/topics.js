...
const VALUABLE_STATUSES = new Set([
  // New: reply cards are the primary signal; reply-title titles follow.
  "reply",
  // Legacy (pre-rewire): still counts as "don't purge" so an operator
  // running `topics purge` on an old host doesn't accidentally nuke a
  // topic whose only content is old narrative/summary events. When
  // they want those GONE they'll explicitly reprocess the topic.
