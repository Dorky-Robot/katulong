 ...
 * client can scroll through in xterm.js. On attach, the server serializes
 * the shared session headless (current viewport + headless scrollback)
 * and ships the result to the client. Anything older than this scrollback
 * window is dropped by the headless before serialization, even though the
 * RingBuffer itself has 20 MB of history. So this constant — not the
 * RingBuffer — is what the user sees as "how far back can I scroll after
 * I attach".
 */
