...
async function sessionCreate(args, ctx) {
  ...
  try {
    const result = await ctx.daemonRPC({ type: "create-session", name: sessionName.toString() });
...
async function sessionRename(args, ctx) {
  ...
  try {
    const result = await ctx.daemonRPC({ type: "rename-session", oldName, newName: newSessionName.toString() });
    if (result.error) return respond({ error: result.error }, `Error: ${result.error}\n`, args, 1);
