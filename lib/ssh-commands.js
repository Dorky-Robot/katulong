...
/**
 * Format output as JSON if --json flag is present.
 */
function formatOutput(data, args) {
  if (args.includes("--json")) {
    return JSON.stringify(data, null, 2) + "\n";
  }
  return null; // Caller should use text formatting
}
...
async function tokenCreate(args, ctx) {
  ...
  if (jsonOut) return { output: jsonOut, exitCode: 0 };
...
async function tokenList(args, ctx) {
  ...
  if (!state) {
    const jsonOut = formatOutput({ tokens: [] }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    return { output: "No tokens.\n", exitCode: 0 };
  }

  const tokens = state.setupTokens.map(t => {
    const credential = t.credentialId
      ? state.getCredential(t.credentialId)
      : null;
    return {
      id: t.id,
      name: t.name,
  ...
  const output = formatTable(
    ["ID", "NAME", "EXPIRES", "CREDENTIAL"],
    tokens.map(t => [t.id, t.name, formatExpiry(t.expiresAt), t.credentialName || "-"]),
  );
...
async function tokenRevoke(args, ctx) {
  ...
  if (!result.found) {
    const jsonOut = formatOutput({ error: "Token not found" }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 1 };
    return { output: "Token not found.\n", exitCode: 1 };
  }

  log.info("Setup token revoked via SSH", { id, credentialRevoked: !!result.removedCredentialId });

  const jsonOut = formatOutput({ ok: true, credentialRevoked: !!result.removedCredentialId }, args);
  if (jsonOut) return { output: jsonOut, exitCode: 0 };
  return {
    output: result.removedCredentialId
      ? "Token and linked credential revoked.\n"
      : "Token revoked.\n",
    exitCode: 0,
  };
}
...
async function credentialList(args, ctx) {
  ...
  if (!state) {
    const jsonOut = formatOutput({ credentials: [] }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    return { output: "No credentials.\n", exitCode: 0 };
  }
  ...
  const output = formatTable(
    ...
    credentials.map(c => [c.id, c.name, formatRelativeTime(c.createdAt), formatRelativeTime(c.lastUsedAt)]),
  );
  return { output, exitCode: 0 };
}
...
async function credentialRevoke(args, ctx) {
  ...
  try {
    ...
    if (!result.found) {
      const jsonOut = formatOutput({ error: "Credential not found" }, args);
      if (jsonOut) return { output: jsonOut, exitCode: 1 };
      return { output: "Credential not found.\n", exitCode: 1 };
    }
  } catch (err) {
    const jsonOut = formatOutput({ error: err.message }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 1 };
  ...
  const jsonOut = formatOutput({ ok: true }, args);
  if (jsonOut) return { output: jsonOut, exitCode: 0 };
  return { output: "Credential revoked.\n", exitCode: 0 };
}
...
async function sessionList(args, ctx) {
  try {
    ...
    const jsonOut = formatOutput({ sessions }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    ...
    const output = formatTable(
      ["NAME", "CLIENTS", "ALIVE"],
      sessions.map(s => [s.name, String(s.clients || 0), s.alive ? "yes" : "no"]),
    );
    return { output, exitCode: 0 };
  } catch (err) {
    const jsonOut = formatOutput({ error: err.message }, args);
...
async function sessionCreate(args, ctx) {
  ...
  try {
    ...
    if (result.error) {
      const jsonOut = formatOutput({ error: result.error }, args);
      if (jsonOut) return { output: jsonOut, exitCode: 1 };
      return { output: `Error: ${result.error}\n`, exitCode: 1 };
    }
    const jsonOut = formatOutput({ name: result.name }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    return { output: `Session "${result.name}" created.\n`, exitCode: 0 };
  } catch (err) {
    const jsonOut = formatOutput({ error: err.message }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 1 };
    return { output: `Error: ${err.message}\n`, exitCode: 1 };
...
async function sessionKill(args, ctx) {
  ...
  try {
    ...
    const jsonOut = formatOutput({ ok: true }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    return { output: `Session "${name}" killed.\n`, exitCode: 0 };
  } catch (err) {
    const jsonOut = formatOutput({ error: err.message }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 1 };
    return { output: `Error: ${err.message}\n`, exitCode: 1 };
...
async function sessionRename(args, ctx) {
  ...
  try {
    ...
    const jsonOut = formatOutput({ name: result.name }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 0 };
    return { output: `Session renamed to "${result.name}".\n`, exitCode: 0 };
  } catch (err) {
    const jsonOut = formatOutput({ error: err.message }, args);
    if (jsonOut) return { output: jsonOut, exitCode: 1 };
    return { output: `Error: ${err.message}\n`, exitCode: 1 };
...
async function statusCommand(args, ctx) {
  ...
  const jsonOut = formatOutput(data, args);
  if (jsonOut) return { output: jsonOut, exitCode: 0 };
 ...
 * @param {object} ctx - Context with loadState, withStateLock, AuthState, daemonRPC
