...
async function tokenCreate(args, ctx) {
  ...
  if (name.length > 128) {
    return respond({ error: "Token name too long (max 128 characters)" }, "Token name too long (max 128 characters).\n", args, 1);
  }
...
async function tokenRevoke(args, ctx) {
  ...
  let result;
  try {
    result = await ctx.withStateLock((state) => {
    ...
    log.error("Token revoke error", { id, error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
...
async function credentialRevoke(args, ctx) {
  ...
  try {
    ...
    log.error("Credential revoke error", { id, error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
...
async function sessionList(args, ctx) {
  try {
    const result = ctx.sessionManager.listSessions();
    ...
    log.error("Session list error", { error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
}

async function sessionCreate(args, ctx) {
  const raw = args.filter(a => a !== "--json").join(" ");
  if (!raw) {
    return { output: "Usage: katulong session create <name>\n", exitCode: 1 };
  }

  const sessionName = SessionName.tryCreate(raw);
  if (!sessionName) {
    return respond({ error: "Invalid session name" }, "Invalid session name.\n", args, 1);
  }

  try {
    const result = await ctx.sessionManager.createSession(sessionName.toString());
    ...
    log.error("Session create error", { name: sessionName.toString(), error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
}

async function sessionKill(args, ctx) {
  const raw = args.find(a => a !== "--json");
  if (!raw) {
  ...
  try {
    const result = ctx.sessionManager.deleteSession(name);
    if (result.error) return respond({ error: result.error }, `Error: ${result.error}\n`, args, 1);
    return respond({ ok: true }, `Session "${name}" killed.\n`, args);
  } catch (err) {
    log.error("Session kill error", { name, error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
}
...
async function sessionRename(args, ctx) {
  ...
  const [rawOldName, rawNewName] = filtered;

  const oldSessionName = SessionName.tryCreate(rawOldName);
  if (!oldSessionName) {
    return respond({ error: "Invalid session name" }, "Invalid session name.\n", args, 1);
  }
  const oldName = oldSessionName.toString();

  const newSessionName = SessionName.tryCreate(rawNewName);
  if (!newSessionName) {
    return respond({ error: "Invalid session name" }, "Invalid session name.\n", args, 1);
  }

  try {
    const result = await ctx.sessionManager.renameSession(oldName, newSessionName.toString());
    if (result.error) return respond({ error: result.error }, `Error: ${result.error}\n`, args, 1);
    return respond({ name: result.name }, `Session renamed to "${result.name}".\n`, args);
  } catch (err) {
    log.error("Session rename error", { oldName, newName: newSessionName.toString(), error: err.message });
    return respond({ error: "Internal error" }, "Internal error.\n", args, 1);
  }
}
...
async function statusCommand(args, ctx) {
  ...
  const result = ctx.sessionManager.listSessions();
  const ptySessions = (result.sessions || []).length;
 ...
 * @param {object} ctx - Context with loadState, withStateLock, sessionManager
...
export async function executeCommand(str, ctx) {
  ...
  const twoWord = tokens.length >= 3 ? `${tokens[1]} ${tokens[2]}` : null;
  const oneWord = tokens[1];

  if (twoWord && handlers[twoWord]) {
