...
export function createWatchlist({ dataDir }) {
  ...
  return {
    ...
    /**
     * Rewind an entry's cursor to 0 so the processor re-reads the whole
     * transcript on its next cycle. Exists for the "purge old topic log +
     * reprocess from scratch" workflow — callers who need to reset without
     * removing + re-adding the entry (which would clobber addedAt).
     *
     * Returns the updated entry, or null if the uuid isn't tracked.
     */
    reset(uuid) {
      return serialize(() => {
        const all = readWatchlist(filePath);
        const entry = all[uuid];
        if (!entry) return null;
        all[uuid] = { ...entry, lastProcessedLine: 0 };
        writeWatchlist(filePath, all);
        return { ...all[uuid] };
      });
    },
