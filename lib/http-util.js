...
export function getCspHeaders(reportOnly = false, req = null) {
  ...
  // Only trust CF headers when socket is loopback (i.e., traffic from local cloudflared).
  // These headers are forgeable by any HTTP client, so we must verify the source.
  const addr = req?.socket?.remoteAddress || "";
  const isCloudflare = req && isLoopbackAddress(addr) && (
    req.headers['cf-ray'] ||
...
export function createChallengeStore(ttlMs) {
  ...
  return { store, consume, destroy, setMeta, getMeta, deleteMeta, _challenges: challenges, _sweep: sweep };
}
