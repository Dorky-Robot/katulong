 ...
 * 3. Cloudflare Tunnel (CF-Visitor header with scheme=https, or loopback socket + CF-Connecting-IP)
 *
...
export function isHttpsConnection(req) {
  ...
  // 3a. Cloudflare Tunnel: CF-Visitor header contains the client-facing scheme
  if (req.headers["cf-visitor"]) {
    try {
      const cfVisitor = JSON.parse(req.headers["cf-visitor"]);
      if (cfVisitor.scheme === "https") return true;
    } catch {
      // Invalid CF-Visitor header, ignore
    }
  }

  // 3b. Cloudflare Tunnel with custom domain: socket is loopback (from cloudflared)
...
export function createChallengeStore(ttlMs) {
  ...
  return { store, consume, destroy, _challenges: challenges, _sweep: sweep };
}
