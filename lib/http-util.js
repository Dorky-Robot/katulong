...
export function getOriginAndRpID(req) {
  ...
  log.debug("getOriginAndRpID", { host, hostname, proto, origin, rpID: hostname });
  return { origin, rpID: hostname };
}

// --- Public path check ---

const STATIC_EXTS = new Set([".js", ".css", ".png", ".ico", ".webp", ".svg", ".woff2", ".json", ".wasm"]);
 ...
 */
export function getCspHeaders(reportOnly = false, req = null, { flutter = false } = {}) {
  ...
  // Flutter CanvasKit compiles Skia WASM at runtime
  if (flutter) {
  ...
  // CanvasKit needs blob: for texture uploads
  const imgSrc = flutter ? "'self' data: blob:" : "'self' data:";
