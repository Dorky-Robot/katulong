...
export function loadState() {
  ...
  if (raw === null) return _cachedState; // readStateFile already set _cachedState = null
  try {
    ...
    // Unexpected migration error - treat as if file doesn't exist
...
// Returns parsed JSON object, or null if file is missing or corrupt.
// Sets _cachedState = null on failure so loadState() can return early.
...
function migrateState(raw) {
  ...
  let state = AuthState.fromJSON(raw);
