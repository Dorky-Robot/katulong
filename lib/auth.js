...
export async function generateRegistrationOpts(rpName, rpID, origin) {
  log.info("WebAuthn: generating registration options (new user)", { rpName, rpID, origin });
...
export async function generateRegistrationOptsForUser(existingUserID, rpName, rpID, origin) {
  log.info("WebAuthn: generating registration options (existing user)", { rpName, rpID, origin });
  ...
  log.info("WebAuthn: registration options generated (existing user)", {
    rpID,
    hasChallenge: !!opts.challenge,
    authenticatorSelection: opts.authenticatorSelection,
    pubKeyCredParams: opts.pubKeyCredParams?.map(p => p.alg),
...
export async function verifyRegistration(response, expectedChallenge, origin, rpID) {
  log.info("WebAuthn: verifying registration response", { origin, rpID });
  try {
    ...
    log.info("WebAuthn: registration verification result", {
    ...
    log.info("WebAuthn: registration credential extracted", {
    ...
    log.error("WebAuthn: registration verification failed", { error: err.message, origin, rpID });
    throw err;
  }
}

export async function generateAuthOpts(credentials, rpID) {
  log.info("WebAuthn: generating authentication options", {
    rpID,
  ...
  log.info("WebAuthn: authentication options generated", {
    rpID,
    hasChallenge: !!opts.challenge,
    allowCredentials: opts.allowCredentials?.length,
  });
  return opts;
}
...
export async function verifyAuth(response, credential, expectedChallenge, origin, rpID) {
  ...
  try {
    ...
    log.info("WebAuthn: authentication verification result", {
      verified: verification.verified,
