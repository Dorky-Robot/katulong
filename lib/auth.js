...
export async function generateRegistrationOpts(rpName, rpID, origin) {
  log.debug("WebAuthn: generating registration options (new user)", { rpID });
  ...
  log.debug("WebAuthn: registration options generated", { rpID });
...
export async function generateRegistrationOptsForUser(existingUserID, rpName, rpID, origin) {
  log.debug("WebAuthn: generating registration options (existing user)", { rpID });
  ...
  log.debug("WebAuthn: registration options generated (existing user)", { rpID });
  return { opts, userID: existingUserID };
}

export async function verifyRegistration(response, expectedChallenge, origin, rpID) {
  log.debug("WebAuthn: verifying registration response", { rpID });
  try {
    ...
    log.error("WebAuthn: registration verification failed", { error: err.message });
    throw err;
  }
}

export async function generateAuthOpts(credentials, rpID) {
  log.debug("WebAuthn: generating authentication options", { rpID, credentialCount: credentials.length });
...
export async function verifyAuth(response, credential, expectedChallenge, origin, rpID) {
  log.debug("WebAuthn: verifying authentication response", { credentialId: credential.id, rpID });
  try {
