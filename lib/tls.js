...
export function generateCA(instanceName, instanceId) {
  ...
  // Include instance ID in CN to make each CA unique and distinguishable in Keychain
  // Format CN: "InstanceName Local CA (shortid)"
  // Format Org: "InstanceName"
  const shortId = instanceId.substring(0, 8);

  const attrs = [
    { name: "organizationName", value: instanceName },
    { name: "commonName", value: `${instanceName} Local CA (${shortId})` },
  ];
