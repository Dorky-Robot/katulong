...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/upload", handler: auth(csrf(async (req, res) => {
      ...
      // Must await — client sends Ctrl+V on response, clipboard must be ready.
      const containerBridged = await bridgeClipboardToContainers(filename, imageMimeType(ext), log);
