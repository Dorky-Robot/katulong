...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/upload", handler: auth(csrf(async (req, res) => {
      ...
      // On Linux, use xclip if available (requires Xvfb for headless containers).
      let clipboard = false;
      if (process.platform === "darwin") {
        ...
        try {
          await new Promise((resolve, reject) => {
            const proc = execFile("xclip", ["-selection", "clipboard", "-t", mimeType, "-i", filePath],
              { timeout: 5000 }, (err) => err ? reject(err) : resolve());
            proc.stdin?.end();
          });
          clipboard = true;
        } catch (err) {
          log.debug("xclip not available — clipboard paste disabled", { error: err.message });
        }
      }
