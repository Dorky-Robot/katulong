...
import { detectPaneContainer, bridgeClipboardToContainer } from "./container-detect.js";
...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/upload", handler: auth(csrf(async (req, res) => {
      ...
      log.info("Upload clipboard status", { hostClipboard: clipboard, kuboBridged: containerBridged, filename });

      // Bridge to the container the user has docker-exec'd into (if any)
      if (!containerBridged) {
        const sessionHeader = req.headers["x-session"];
        log.info("Pane container detection", { sessionHeader, hasSession: !!sessionHeader });
        if (sessionHeader) {
          const session = sessionManager.getSession(sessionHeader);
          log.info("Session lookup", { found: !!session, tmuxName: session?.tmuxName });
          if (session) {
            const container = await detectPaneContainer(session.tmuxName);
            log.info("Pane container detected", { container });
            if (container) {
              const paneBridged = await bridgeClipboardToContainer(container, filePath, imageMimeType(ext), log);
              log.info("Pane container bridge result", { paneBridged });
    ...
    // --- Paste (set clipboard to an uploaded image) ---
    //
    // Called by the client after upload to set the clipboard and trigger paste.
    // Separated from upload so multiple files can upload in parallel, then
    // paste sequentially without clipboard contention.
    ...
    { method: "POST", path: "/paste", handler: auth(csrf(async (req, res) => {
      ...
      (async () => {
        for (const p of paths) {
          ...
          let clipboard = await setClipboard(filePath, ext, log);
          const bridged = await bridgeClipboardToContainers(filename, imageMimeType(ext), log);
          if (bridged) clipboard = true;

          // Bridge to docker-exec'd container in this session's pane
          if (!bridged && data.session) {
            const session = sessionManager.getSession(data.session);
            log.info("Paste: pane container detection", { session: data.session, found: !!session, tmuxName: session?.tmuxName });
            if (session) {
              const container = await detectPaneContainer(session.tmuxName);
              log.info("Paste: pane container detected", { container });
              if (container) {
                const paneBridged = await bridgeClipboardToContainer(container, filePath, imageMimeType(ext), log);
                log.info("Paste: pane container bridge result", { paneBridged });
                if (paneBridged) clipboard = true;
              }
            }
          }

          if (clipboard && data.session) {
            const session = sessionManager.getSession(data.session);
