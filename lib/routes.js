...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "GET", prefix: "/sessions/", handler: auth(async (req, res, param) => {
      ...
      if (screen === "true") {
        // serializeScreen is async; the previous code forgot to await it
        // and JSON.stringify serialized the Promise as `{}`, so the `screen`
        // field was silently always empty. Boy Scout fix while touching the
        // serialize paths in PCH-7.
