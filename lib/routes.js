...
export function createAuthRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/auth/register/options", rateLimit: true, handler: async (req, res) => {
      const { origin: reqOrigin, rpID: reqRpID } = getOriginAndRpID(req);
      log.info("POST /auth/register/options: request received", {
        ip: req.socket.remoteAddress,
        host: req.headers.host,
        origin: req.headers.origin,
        derivedOrigin: reqOrigin,
        derivedRpID: reqRpID,
      ...
      log.info("POST /auth/register/options: parsed body", { hasSetupToken: !!setupToken });

      let state = loadState();
      if (!state) {
        log.info("POST /auth/register/options: no existing state, creating empty state");
      ...
      log.info("POST /auth/register/options: registration context", {
        isFirstRegistration,
        isLocal,
      ...
      if (isFirstRegistration && isLocal) {
        ...
        if (!tokenData) {
          log.warn("POST /auth/register/options: invalid setup token", {
            ...
            availableTokenCount: state?.setupTokens?.length ?? 0,
          });
          return json(res, 403, { error: "Invalid setup token" });
        }
        log.info("POST /auth/register/options: setup token validated", { tokenId: tokenData.id });
      ...
      if (isSetup()) {
        if (state.user && state.user.id) {
          log.info("POST /auth/register/options: generating opts for existing user");
          ({ opts, userID } = await generateRegistrationOptsForUser(state.user.id, RP_NAME, rpID, origin));
        } else {
          log.info("POST /auth/register/options: generating opts (no existing user ID)");
          ({ opts, userID } = await generateRegistrationOpts(RP_NAME, rpID, origin));
        }
      } else {
        log.info("POST /auth/register/options: generating opts for first-time setup");
        ({ opts, userID } = await generateRegistrationOpts(RP_NAME, rpID, origin));
      }
      storeChallenge(opts.challenge);
      challengeStore.setMeta(opts.challenge, "userID", userID);
      log.info("POST /auth/register/options: challenge stored, sending options to client", {
        rpID, origin,
    ...
    { method: "POST", path: "/auth/register/verify", rateLimit: true, handler: async (req, res) => {
      ...
      log.info("POST /auth/register/verify: parsed body", {
      ...
      log.info("POST /auth/register/verify: challenge consumed", { challengeValid });

      const userID = challengeStore.getMeta(challenge, "userID");
      challengeStore.deleteMeta(challenge, "userID");
      log.info("POST /auth/register/verify: userID retrieved", { hasUserID: !!userID });
      ...
      const result = await withStateLock(async (currentState) => {
        ...
        if (setupToken) {
          ...
          log.info("POST /auth/register/verify: setup token validated in lock", { tokenId: setupTokenId });
        }
      ...
      log.info("POST /auth/register/verify: session cookie set", {
        secure: isHttpsConnection(req),
      });
      ...
      log.info("POST /auth/register/verify: registration complete — success");
    ...
    { method: "POST", path: "/auth/login/options", rateLimit: true, handler: async (req, res) => {
      ...
      log.info("POST /auth/login/options: state loaded", {
        credentialCount: state.credentials.length,
        credentialIds: state.credentials.map(c => c.id),
      });

      const opts = await generateAuthOpts(state.credentials, rpID);
      storeChallenge(opts.challenge);
      log.info("POST /auth/login/options: options sent to client", {
        rpID, allowCredentials: opts.allowCredentials?.length,
      });
      json(res, 200, opts);
    }},
    ...
    { method: "POST", path: "/auth/login/verify", rateLimit: true, handler: async (req, res) => {
      ...
      log.info("POST /auth/login/verify: parsed body", {
        credentialId: credential?.id,
        credentialType: credential?.type,
        hasClientDataJSON: !!credential?.response?.clientDataJSON,
      ...
      const challenge = extractChallenge(credential);
      const challengeValid = consumeChallenge(challenge);
      log.info("POST /auth/login/verify: challenge consumed", { challengeValid });
      ...
      const { session } = result.result.data;
      setSessionCookie(res, session.token, session.expiry, { secure: isHttpsConnection(req) });
      log.info("POST /auth/login/verify: login complete — success", {
        credentialId: credential.id,
        secure: isHttpsConnection(req),
      });
      json(res, 200, { ok: true });
