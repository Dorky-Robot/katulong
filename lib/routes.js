...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/attach", handler: auth(csrf(async (req, res) => {
      ...
      const created = !sessionManager.getSession(sessionName.toString());
      if (created) {
        const result = await sessionManager.createSession(sessionName.toString(), 120, 40);
        if (result.error) return json(res, 409, { error: result.error });
      }
      bridge.relay({ type: "open-tab", session: sessionName.toString() });
      // Send command to the session if provided
      if (typeof data.command === "string" && data.command.trim()) {
        const session = sessionManager.getSession(sessionName.toString());
        if (session?.alive) {
          // Brief delay for new sessions so shell has time to init
          setTimeout(() => session.write(data.command.trim() + "\n"), created ? 500 : 50);
        }
      }
      json(res, 200, { name: sessionName.toString(), created });
    }))},
