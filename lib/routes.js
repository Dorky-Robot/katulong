...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/sessions", handler: auth(csrf(async (req, res) => {
      ...
      const rpc = { type: "create-session", name: sessionName.toString() };
      if (copyFrom) {
        const copyFromName = SessionName.tryCreate(copyFrom);
        if (copyFromName) rpc.copyFrom = copyFromName.toString();
      }
      const result = await daemonRPC(rpc);
