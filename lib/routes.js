...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/upload", handler: auth(csrf(async (req, res) => {
      ...
      // On Linux (e.g., inside a kubo container), the file is already in the uploads dir
      // which is mounted into containers — pbpaste shims (like kubo's) find it via ls -t.
      let clipboard = false;
      if (process.platform === "darwin") {
        ...
        // No native clipboard, but the image is saved to the uploads dir.
        // Container pbpaste shims (kubo) read the latest file from this dir.
        clipboard = true;
      }
    ...
    { method: "GET", path: "/api/claude-sessions", handler: auth((req, res) => {
      json(res, 200, { sessions: claudeSessionManager.listSessions() });
    })},
