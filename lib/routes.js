 ...
 *
 * This runs async fire-and-forget — clipboard bridge failures should
 * never block the upload response.
 */
function bridgeClipboardToContainers(filename, mimeType, log) {
  execFile("docker", [
    ...
    for (const name of containers) {
      execFile("docker", [
        ...
        "-i", `/home/dev/.katulong/uploads/${filename}`
      ], { timeout: 5000 }, (execErr) => {
        if (execErr) {
          log.debug("Container clipboard bridge failed", { container: name, error: execErr.message });
        } else {
...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    { method: "POST", path: "/upload", handler: auth(csrf(async (req, res) => {
      ...
      // On macOS, use osascript to set the native clipboard.
      // On Linux, use xclip if available. Auto-detects Xvfb display when DISPLAY is unset
      // (headless containers start Xvfb on :99 but may not export DISPLAY to all processes).
      // Also propagates DISPLAY into tmux's global environment so child processes (Claude Code)
      ...
      if (process.platform === "darwin") {
        ...
        // Auto-detect Xvfb display if DISPLAY is not set in our environment.
        // Headless containers (kubo) start Xvfb on :99 via entrypoint.sh, but
        // the env var may not propagate to katulong or tmux child processes.
        if (!process.env.DISPLAY) {
          try {
            // Check if Xvfb is running and which display it uses
            ...
            if (xvfbDisplay) {
              ...
              // Propagate DISPLAY into tmux's global env so Claude Code
              // (and any process started in a tmux session) can read the clipboard.
              execFile("tmux", ["setenv", "-g", "DISPLAY", xvfbDisplay], { timeout: 2000 }, () => {});
            }
          } catch { /* Xvfb not running — fall through to xclip which will fail gracefully */ }
        }
        const mimeType = imageMimeType(ext);
        try {
          // xclip -i accepts [FILE]... as positional args (reads from files, not just stdin)
          await new Promise((resolve, reject) => {
      ...
      // Bridge image clipboard to running kubo containers.
      // kubo containers have their own Xvfb (:99) and xclip, so the host
      // clipboard (set above) is isolated from the container clipboard.
      // The uploads dir is mounted at /home/dev/.katulong/uploads, so we
      // can use docker exec to copy the image to each container's clipboard.
      bridgeClipboardToContainers(filename, imageMimeType(ext), log);
