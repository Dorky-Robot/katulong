...
export function createAppRoutes(ctx) {
  ...
  return [
    ...
    // Hook endpoint — Claude Code hooks POST events here.
    // Auth-protected so only localhost or authenticated clients can post.
    // This is how the browser mirrors what Claude Code is doing in the terminal.
    { method: "POST", path: "/api/helm/hook", handler: auth(async (req, res) => {
      const event = await parseJSON(req);
      bridge.relay({
        type: "helm-hook-event",
        event,
      });
      json(res, 200, { ok: true });
    })},
