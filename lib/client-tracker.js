...
export function createClientTracker({ bridge, getSession }) {
  ...
  function attach(clientId, sessionName, cols, rows) {
    ...
    // force:true bypasses the output-idle gate; otherwise the snapshot
    // taken right after this in attachClient() captures the headless at
    // stale dims and garbles absolute cursor escapes on the client.
    if (cols && rows) {
      const session = getSession(sessionName);
      if (session?.alive) session.resize(cols, rows, { force: true });

      if (clientCount(sessionName) > 1) {
        bridge.relay({
          type: "resize-sync",
          session: sessionName,
          cols,
    ...
    if (wasActive) {
      ...
      if (newActiveId) {
        ...
        if (newInfo?.cols && newInfo?.rows) {
          ...
          if (session?.alive) {
            // Active-client promotion is a discrete lifecycle event; force
            // past the idle gate so the new active dims take effect now.
            session.resize(newInfo.cols, newInfo.rows, { force: true });
          }
          bridge.relay({
            type: "resize-sync",
