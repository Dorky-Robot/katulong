...
// cycle. Rolling summary + recent excerpt means older context survives
// via the previous summary, so this window can stay tight.
const SUMMARY_CONTEXT_LIMIT = 60;
...
export function createClaudeProcessor({
  ...
  function formatForSummary(entries) {
    const out = [];
    for (const e of entries) {
      if (e.role === "user" && e.text) out.push(`User: ${truncateForSummary(e.text, 500)}`);
      else if (e.role === "assistant" && e.text) out.push(`Claude: ${truncateForSummary(e.text, 800)}`);
      else if (e.role === "tool_result" && e.text) out.push(`Tool result: ${truncateForSummary(e.text, 200)}`);
    }
    return out.join("\n\n");
  }
