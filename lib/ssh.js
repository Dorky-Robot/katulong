...
import { executeCommand } from "./ssh-commands.js";
...
/**
 * Start an SSH server that bridges terminal sessions via the session manager.
 *
 * @param {object} opts
 ...
 * @param {object} opts.authContext - Auth context { loadState, withStateLock } for CLI commands
 * @returns {{ server: Server }}
 */
export function startSSHServer({ port, hostKey, password, sessionManager, credentialLockout, bridge, authContext }) {
    ...
    client.on("ready", () => {
      ...
      client.on("session", (accept) => {
        ...
        function handleShell(accept) {
          ...
          sessionManager.attachClient(clientId, sessionName, ptyInfo.cols, ptyInfo.rows).then((result) => {
            ...
            stream.on("data", (data) => {
              sessionManager.writeInput(clientId, data.toString());
            });

            session.on("window-change", (accept, _reject, info) => {
              sessionManager.resizeClient(clientId, info.cols, info.rows);
              if (accept) accept();
            });
        ...
        // Intercept "katulong <cmd>" exec requests for CLI commands.
        // Non-katulong exec requests fall through to handleShell (full PTY) —
        // SSH users are password-authenticated and have equivalent shell access.
        session.on("exec", (accept, _reject, info) => {
          if (authContext) {
            const commandCtx = {
              loadState: authContext.loadState,
              withStateLock: authContext.withStateLock,
              sessionManager,
            };
            executeCommand(info.command, commandCtx).then((result) => {
              if (result) {
                const stream = accept();
              ...
              log.error("SSH command error", { command: info.command, error: err.message });
              const stream = accept();
