...
export function validateP2PSignal(msg) {
  ...
  // Validate SDP fields before they reach native node-datachannel bindings
  if (msg.data.type !== undefined && !isString(msg.data.type)) {
    return { valid: false, error: "data.type must be a string" };
  }
  if (msg.data.sdp !== undefined && !isString(msg.data.sdp)) {
    return { valid: false, error: "data.sdp must be a string" };
  }
  if (msg.data.candidate !== undefined) {
    if (!isObject(msg.data.candidate)) {
      return { valid: false, error: "data.candidate must be an object" };
    }
