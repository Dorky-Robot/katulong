...
/**
 * Ensure the dispatch workspace exists with pre-configured Claude permissions.
 * This directory is where refine sessions run — Claude inherits its settings.
 * Returns the absolute path.
 */
function ensureDispatchWorkspace() {
  const wsDir = join(homedir(), ".katulong", "dispatch-workspace");
  ...
  if (!existsSync(claudeMdPath)) {
    writeFileSync(claudeMdPath, `# Dispatch Workspace

This is the working directory for katulong's dispatch refinement sessions.
You are triaging raw feature ideas into implementable tickets.

## Available tools

- \`diwa ls\` — list all indexed projects
- \`diwa search <project> "<query>"\` — search project history
- Read project CLAUDE.md files for architecture and constraints
- Write JSON output files as instructed in the prompt

## Guidelines

- Be thorough: read CLAUDE.md and search diwa for each relevant project
- Consolidate overlapping ideas into single tickets
- Split cross-cutting ideas into per-project tickets when appropriate
- Flag vague ideas as needs-info with clear explanation of what's missing
`);
  }

  // Write settings.local.json with pre-allowed permissions
...
export function createRefiner({ sessionManager } = {}) {
  return {
     ...
     */
    async refineBatch(store, featureIds, { sessionTag, onSessionCreated } = {}) {
      ...
      // Link session to all grouped features and notify caller
      for (const f of features) {
        await store.updateFeature(f.id, { sessionName });
      }
      if (onSessionCreated) onSessionCreated(featureIds, sessionName);

      // Write prompt to file and run Claude Code in the session
      const promptFile = join(tmpdir(), `dispatch-refine-${sessionTag}.txt`);
      writeFileSync(promptFile, prompt);

      // Ensure dispatch workspace with pre-configured Claude permissions
      const workspace = ensureDispatchWorkspace();

      const session = sessionManager.getSession(sessionName);
      session.write(`cd ${workspace} && claude "$(cat ${promptFile})"\r`);
