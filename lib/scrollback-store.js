...
export function createScrollbackStore({ dataDir }) {
  ...
  function load(sessionId) {
    ...
    try {
      ...
      // ENOENT = no scrollback persisted yet (normal). Anything else means
      // scrollback exists but can't be read — surface it instead of
      // presenting an empty terminal as if nothing was ever saved.
      if (err.code !== "ENOENT") {
        log.warn("Failed to stat scrollback file", { sessionId, error: err.message });
      }
      return null;
    }
    ...
    try {
      ...
      if (err.code !== "ENOENT") {
        log.warn("Failed to read scrollback file", { sessionId, error: err.message });
      }
      return null;
    }
