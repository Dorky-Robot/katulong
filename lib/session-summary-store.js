...
export function createSummaryStore({ dataDir }) {
  ...
  function readFile(filePath) {
    ...
    try {
      ...
      // ENOENT = no summaries yet (normal). Anything else hides existing
      // history behind what looks like an empty file — log it.
      if (err.code !== "ENOENT") {
        log.warn("Failed to stat summary file", { filePath, error: err.message });
      }
      return [];
    }
    ...
    try {
      ...
      if (err.code !== "ENOENT") {
        log.warn("Failed to read summary file", { filePath, error: err.message });
      }
      return [];
    }
    const out = [];
