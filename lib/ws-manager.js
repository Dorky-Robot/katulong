 ...
 */
export function createWebSocketManager({ bridge, sessionManager, claudeSessionManager }) {
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      ...
      case "claude-mode-changed":
        sendToSession(msg.session, {
          type: "claude-mode-changed", session: msg.session,
      ...
      case "claude-event":
        sendToSession(msg.session, {
          type: "claude-event", session: msg.session, event: msg.event,
        });
        break;
      case "claude-turn-complete":
        sendToSession(msg.session, { type: "claude-turn-complete", session: msg.session });
        break;
      case "claude-waiting-for-input":
        sendToSession(msg.session, { type: "claude-waiting-for-input", session: msg.session });
        break;
      ...
      const wsMessageHandlers = {
        ...
        "claude-input"() {
          if (claudeSessionManager) {
            const delivered = claudeSessionManager.sendUserMessage(msg.session, msg.content);
            if (!delivered) {
              ws.send(JSON.stringify({ type: "error", message: "No active claude session for this terminal" }));
            }
          }
        },
        "claude-tool-response"() {
          if (claudeSessionManager) {
            claudeSessionManager.sendToolResponse(msg.session, msg.id, msg.approved);
          }
        },
        "claude-abort"() {
          if (claudeSessionManager) {
            claudeSessionManager.abortSession(msg.session);
          }
        },
        "p2p-signal"() {
          const info = wsClients.get(clientId);
