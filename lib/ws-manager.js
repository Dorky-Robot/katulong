...
export function createWebSocketManager({ bridge, sessionManager, helmSessionManager }) {
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      ...
      // Broadcast to all clients — the agent may not know the browser's
      // session name, so the browser resolves the match itself.
      case "helm-mode-changed":
        broadcastToAll({
          type: "helm-mode-changed", session: msg.session,
          active: msg.active, agent: msg.agent, prompt: msg.prompt, cwd: msg.cwd,
      ...
      case "helm-turn-complete":
        broadcastToAll({ type: "helm-turn-complete", session: msg.session });
        break;
      case "helm-waiting-for-input":
        broadcastToAll({ type: "helm-waiting-for-input", session: msg.session });
        break;

      // Claude Code hook events (from /api/helm/hook endpoint)
      case "helm-hook-event":
        broadcastToAll({ type: "helm-hook-event", event: msg.event });
        break;
  ...
  function handleConnection(ws) {
    ...
    async function handleMessage(raw) {
      ...
      const wsMessageHandlers = {
        ...
        "helm-input"() {
          // Try the WebSocket-based helm session first (yolo Agent SDK mode)
          if (helmSessionManager) {
            const delivered = helmSessionManager.sendUserMessage(msg.session, msg.content);
            if (delivered) return;
          }
          // Fallback: inject text into the terminal session directly.
          // This handles the hook-based companion mode where Claude Code
          // runs in the terminal TUI — typing in helm types in the terminal.
          if (msg.session && msg.content) {
            const session = sessionManager.getSession(msg.session);
            if (session?.alive) {
              session.write(msg.content + "\r");
            }
          }
        },
        "helm-tool-response"() {
