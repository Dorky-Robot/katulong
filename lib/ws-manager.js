...
export function createWebSocketManager({ bridge, sessionManager, helmSessionManager }) {
  ...
  function sendToSession(sessionName, payload, { preferP2P = false } = {}) {
    ...
    for (const [clientId, info] of wsClients) {
      ...
      if (preferP2P && info.p2pConnected && info.p2pPeer) {
        if (info.p2pPeer.send(encoded)) continue;
        // P2P send failed (channel not open) — fall through to WebSocket
      }
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      // Session I/O routing
      case "output":
        sendToSession(msg.session, { type: "output", session: msg.session, data: msg.data, seq: msg.seq }, { preferP2P: true });
        break;
      case "exit":
      ...
      const wsMessageHandlers = {
        async attach() {
          ...
          try {
            ...
            // Tell client where live output starts (scrollback replay is opaque, not sequenced)
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
            // Register in routing table AFTER seq-init so bridge output
            // doesn't arrive before the client's sequence buffer is initialized.
            // Any output in the gap is recovered via the catchup mechanism.
        ...
        async switch() {
          ...
          const info = wsClients.get(clientId);
          try {
            // Remove from routing during switch to prevent bridge output
            // for the new session arriving before seq-init — the client's
            // seqBuffer still has the old session's state and would
            // misinterpret the sequence numbers, causing garbled text.
            ...
            // Tell client where live output starts for this session
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
            // Re-add to routing AFTER seq-init — any output in the gap
            // is recovered via the catchup mechanism.
            if (info) wsClients.set(clientId, info);
        ...
        "seq-query"() {
          const sessionName = sessionManager.getSessionForClient(clientId);
          if (!sessionName) return;
          const session = sessionManager.getSession(sessionName);
          if (!session) return;
          ws.send(JSON.stringify({ type: "seq-status", session: sessionName, seq: session.outputBuffer.totalBytes }));
        },
        catchup() {
          // Always use the client's attached session — never allow cross-session catchup
          const attachedName = sessionManager.getSessionForClient(clientId);
          if (!attachedName) return;
          const name = msg.session ? resolveSessionName(msg.session) : attachedName;
          if (!name || name !== attachedName) return;
          const session = sessionManager.getSession(name);
          if (!session) return;
          // Apply backpressure check — skip catchup for slow clients
          if (ws.bufferedAmount > WS_BACKPRESSURE_BYTES) {
            log.debug("Dropping catchup for slow client", { clientId, buffered: ws.bufferedAmount });
            return;
          }
          const data = session.outputBuffer.sliceFrom(msg.fromSeq);
          if (data === null) {
            // Data has been evicted — client must re-attach for a full replay
            ws.send(JSON.stringify({ type: "seq-reset", session: name }));
          } else if (data.length > 0) {
            ws.send(JSON.stringify({ type: "catchup-data", session: name, data, seq: msg.fromSeq }));
          }
          // If data is empty string, client is already caught up — no response needed
        },
        "set-tab-icon"() {
          const name = resolveSessionName(msg.session);
          if (!name) return;
        ...
        "p2p-signal"() {
          ...
          if (!info.p2pPeer) {
            const thisPeer = createServerPeer(
              (data) => { if (ws.readyState === 1) ws.send(JSON.stringify({ type: "p2p-signal", data })); },
              (chunk) => {
                try {
                  const str = typeof chunk === "string" ? chunk : chunk.toString("utf8");
                  const p2pMsg = JSON.parse(str);
                  if (p2pMsg.type === "input" && typeof p2pMsg.data === "string") {
                    const session = typeof p2pMsg.session === "string" ? resolveSessionName(p2pMsg.session) : undefined;
                    sessionManager.writeInput(clientId, p2pMsg.data, session);
                  }
                } catch (err) { log.warn("Malformed P2P data", { clientId, error: err.message }); }
              },
              () => {
                // Guard: only null out if this peer is still the active one.
                // Prevents a stale onClose from a destroyed peer clobbering a newly installed peer.
                const cur = wsClients.get(clientId);
                if (cur && cur.p2pPeer === thisPeer) { cur.p2pPeer = null; cur.p2pConnected = false; }
                if (ws.readyState === 1) ws.send(JSON.stringify({ type: "p2p-closed" }));
              },
              () => {
                const cur = wsClients.get(clientId);
                if (cur && cur.p2pPeer === thisPeer) cur.p2pConnected = true;
                if (ws.readyState === 1) ws.send(JSON.stringify({ type: "p2p-ready" }));
              }
            );
            info.p2pPeer = thisPeer;
