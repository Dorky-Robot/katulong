    ...
    async function handleMessage(raw) {
      ...
      const wsMessageHandlers = {
        ...
        async pull() {
          ...
          if (data === null) {
            // Cursor evicted — use per-client headless for recovery snapshot
            // (falls back to shared headless if per-client not available)
            const ch = sessionManager.getClientHeadless?.(clientId, name);
            const snapshot = ch
        ...
        async resync() {
          // Drift detected by client — use per-client headless for snapshot
          // (falls back to shared headless if per-client not available)
          const name = msg.session ? resolveSessionName(msg.session) : sessionManager.getSessionForClient(clientId);
          if (!name || !sessionManager.isClientSubscribedTo(clientId, name)) return;
          const session = sessionManager.getSession(name);
