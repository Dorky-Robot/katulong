  ...
  bridge.register((msg) => {
    switch (msg.type) {
      ...
      case "session-renamed":
        // Session binding is already updated by the tracker (via session-manager.renameSession)
        // before this event is relayed, so clients are now under the new name.
      ...
      case "child-count-update":
        ...
        break;
      case "resize-sync":
        // Send to all clients on this session EXCEPT the active one
        for (const [clientId, info] of wsClients) {
          if (sessionManager.getSessionForClient(clientId) !== msg.session) continue;
          if (clientId === msg.activeClientId) continue;
          if (info.ws.readyState === 1) {
