...
export function createWebSocketManager({ bridge, sessionManager, helmSessionManager, pluginWsHandlers = {} }) {
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      // Session I/O routing — lightweight notification (no payload)
      case "data-available":
        sendToSession(msg.session, { type: "data-available", session: msg.session });
        break;
      case "exit":
      ...
      const wsMessageHandlers = {
        ...
        async pull() {
          ...
          if (ws.bufferedAmount > WS_BACKPRESSURE_BYTES) {
            log.debug("Dropping pull response for slow client", { clientId, buffered: ws.bufferedAmount });
            return;
          }
          const data = session.outputBuffer.sliceFrom(msg.fromSeq);
          if (data === null) {
            // Cursor evicted — send a snapshot of the current pane state
            // so the client can recover without reconnecting.
            const { captureVisiblePane } = await import("./tmux.js");
