      ...
      const wsMessageHandlers = {
        ...
        async switch() {
          ...
          try {
            ...
            ws.send(JSON.stringify({ type: "switched", session: name, data: result.buffer || "" }));
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
        ...
        async subscribe() {
          ...
          try {
            ...
            // Snapshot only — no seq-init, no live pulling for background tiles.
            // Live pull data uses cursor positioning for the PTY's row count,
            // which may differ from the carousel card's row count, causing garble.
            // Background tiles show the snapshot; switching to them gets a fresh
            // snapshot + seq-init via the switch handler.
