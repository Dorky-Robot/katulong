 ...
 */
export function createWebSocketManager({ bridge, sessionManager, pluginWsHandlers = {} }) {
  ...
  function handleConnection(ws, auth = {}) {
    ...
    let lastAuthCheck = 0;
    // Set by the close/error handlers. Async handlers (attach/switch) that
    // were in-flight when the socket closed re-insert this client into
    // wsClients after their await resumes — cleanupClient already ran and
    // found nothing to clean, so the re-inserted entry would be stranded
    // forever (zombie transport, tracker entry never detached). Handlers
    // check this flag after they finish and re-run cleanup if needed.
    let closed = false;
    ...
    async function handleMessage(raw) {
      ...
      const wsMessageHandlers = {
        ...
        async pull() {
          ...
          if (data === null) {
            ...
            // size; the shared headless is written live at current PTY dims.
        ...
        async resync() {
          // Drift detected by client — snapshot from the shared session
          // headless (Lamport-correct { buffer, seq } pair).
          const name = msg.session ? resolveSessionName(msg.session) : sessionManager.getSessionForClient(clientId);
          if (!name || !sessionManager.isClientSubscribedTo(clientId, name)) return;
          const session = sessionManager.getSession(name);
        ...
        // Application-level ping/pong — proves the full message-handling
        // stack is alive (needed for DataChannel which has no native ping,
        // and JS can't observe WS protocol pings).
        // Must respond via the active transport — if the client is on DC,
        // a pong sent over WS gets dropped by the client transport layer.
        ping() {
          if (transport.readyState === 1) transport.send(JSON.stringify({ type: "pong" }));
        },
      ...
      if (handler) {
        try {
          ...
          // Socket closed while this handler's await was in flight — the
          // close event's cleanupClient ran before the handler re-inserted
          // the routing entry. Clean up again so nothing is stranded.
          if (closed && wsClients.has(clientId)) cleanupClient(clientId);
        }
        ...
        try {
          ...
          // Plugins can't insert into wsClients today, but keep the same
          // closed-during-await guard as the built-in branch so a future
          // plugin capability can't reintroduce the zombie-entry race.
          if (closed && wsClients.has(clientId)) cleanupClient(clientId);
        }
      }
    }
