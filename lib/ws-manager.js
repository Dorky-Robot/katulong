      ...
      const wsMessageHandlers = {
        ...
        async subscribe() {
          ...
          try {
            ...
            // Don't send buffer on subscribe — the terminal pool already
            // has cached content, and sending it again duplicates output.
            // The pull mechanism delivers any new data from seq-init onward.
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
            if (!result.alive) {
