...
export function createWebSocketManager({ bridge, sessionManager, helmSessionManager, pluginWsHandlers = {} }) {
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      // Session I/O routing
      case "output":
        sendToSession(msg.session, { type: "output", session: msg.session, data: msg.data, seq: msg.seq });
        break;
      case "exit":
  ...
  function handleConnection(ws) {
    ...
    async function handleMessage(raw) {
      ...
      const wsMessageHandlers = {
        ...
        "seq-query"() {
          // Send seq-status for every session this client is receiving
          // output from (primary + subscriptions) so per-session nudge
          // catchup works across all channels.
          const primaryName = sessionManager.getSessionForClient(clientId);
          const subs = sessionManager.getSubscriptionsForClient(clientId);
          const names = new Set(subs);
          if (primaryName) names.add(primaryName);
          for (const name of names) {
            const session = sessionManager.getSession(name);
            if (session) {
              ws.send(JSON.stringify({ type: "seq-status", session: name, seq: session.outputBuffer.totalBytes }));
            }
          }
        },
        catchup() {
          // Allow catchup for any session the client is subscribed to
          // (primary attachment or explicit subscription).
        ...
        async subscribe() {
          ...
          try {
            ...
            // Send scrollback buffer so carousel tiles show existing content
            if (result.buffer) ws.send(JSON.stringify({ type: "output", session: name, data: result.buffer }));
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
