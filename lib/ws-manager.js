        ...
        async attach() {
          ...
          try {
            ...
            if (result.seq !== undefined) {
              const seqInit = { type: "seq-init", session: name, seq: result.seq };
              if (result.cursor) { seqInit.cursorRow = result.cursor.row; seqInit.cursorCol = result.cursor.col; }
              ws.send(JSON.stringify(seqInit));
            }
