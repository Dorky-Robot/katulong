...
export function createWebSocketManager({ bridge, sessionManager, helmSessionManager, pluginWsHandlers = {} }) {
  ...
  bridge.register((msg) => {
    switch (msg.type) {
      // Session I/O routing — mutation-based terminal updates
      case "snapshot": {
        const update = { type: "term-update", session: msg.session };
        if (msg.snapshotType === "cursor") {
          update.action = "cursor";
        } else if (msg.snapshotType === "scroll") {
          update.action = "scroll";
          update.count = msg.count;
          update.newLines = msg.newLines;
        } else if (msg.snapshotType === "mutations") {
          update.action = "mutations";
          update.rows = msg.rows;
        } else {
          update.action = "full";
          update.lines = msg.lines;
        }
        update.cursorX = msg.cursorX;
        update.cursorY = msg.cursorY;
        sendToSession(msg.session, update);
        break;
      }
      case "exit":
      ...
      const wsMessageHandlers = {
        async attach() {
          ...
          try {
            ...
            if (result.buffer) {
              ...
              ws.send(JSON.stringify({ type: "term-update", session: name, action: "full", lines }));
            }
        ...
        async subscribe() {
          ...
          try {
            ...
            if (result.buffer) ws.send(JSON.stringify({ type: "term-update", session: name, action: "full", lines: result.buffer.split("\n").filter((_, i, a) => i < a.length - 1 || _ !== "") }));
            if (!result.alive) {
