  ...
  bridge.register((msg) => {
    switch (msg.type) {
      ...
      case "resize-sync":
        ...
        for (const [clientId, info] of wsClients) {
          if (info.session !== msg.session) continue;
      ...
      const wsMessageHandlers = {
        async attach() {
          ...
          try {
            ...
            log.debug("Client attached", { clientId: clientId.slice(0, 8), session: name });
        ...
        async switch() {
          ...
          try {
            ...
            log.debug("Client switched", { clientId: clientId.slice(0, 8), session: name });
