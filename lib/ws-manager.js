    ...
    switch (msg.type) {
      ...
      case "output": {
        ...
        for (const [clientId, info] of wsClients) {
          ...
          if (info.ws.readyState !== 1) continue;
          if (info.ws.bufferedAmount > WS_BACKPRESSURE_BYTES) {
            // Backpressure — send lightweight notification so client can pull
            info.ws.send(JSON.stringify({ type: "data-available", session: msg.session }));
          } else {
            info.ws.send(encoded);
          }
        }
      ...
      case "state-check":
        if (msg.clientId) {
          // Per-client drift detection (PCH-3): send fingerprint only to the
          // specific client whose headless computed it.
          const info = wsClients.get(msg.clientId);
          if (info && info.ws.readyState === 1) {
            info.ws.send(JSON.stringify({
              type: "state-check", session: msg.session, fingerprint: msg.fingerprint,
            }));
          }
        } else {
          // Backward compat: no clientId means broadcast to all session clients
  ...
  function handleConnection(ws) {
    ...
    async function handleMessage(raw) {
      ...
      if (ws.sessionToken) {
        ...
        if (!ws._lastAuthCheck || now - ws._lastAuthCheck >= AUTH_RECHECK_MS) {
      ...
      const wsMessageHandlers = {
        async attach() {
          ...
          try {
            ...
            ws.send(JSON.stringify({ type: "attached", session: name, data: result.buffer || "" }));
            ...
            ws.send(JSON.stringify({ type: "data-available", session: name }));
        ...
        async pull() {
          ...
          if (data === null) {
            // Cursor evicted — serialize headless xterm state for recovery
        ...
        async resync() {
          // Drift detected by client — send a fresh serialize snapshot
          const name = msg.session ? resolveSessionName(msg.session) : sessionManager.getSessionForClient(clientId);
          if (!name || !sessionManager.isClientSubscribedTo(clientId, name)) return;
          const session = sessionManager.getSession(name);
        ...
        async subscribe() {
          ...
          try {
            ...
            ws.send(JSON.stringify({ type: "subscribed", session: name, data: result.buffer || "" }));
