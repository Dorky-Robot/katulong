      ...
      const wsMessageHandlers = {
        ...
        async switch() {
          ...
          try {
            ...
            const result = await sessionManager.attachClient(clientId, name, msg.cols, msg.rows, { skipResize: !!msg.cached });
        ...
        async subscribe() {
          ...
          try {
            ...
            // Send buffer snapshot so freshly created carousel tiles show
            // existing terminal content immediately (e.g., after page refresh).
            // Without this, non-active tiles stay blank until new output arrives.
            if (result.buffer) ws.send(JSON.stringify({ type: "output", session: name, data: result.buffer }));
            if (result.seq !== undefined) {
              ws.send(JSON.stringify({ type: "seq-init", session: name, seq: result.seq }));
            }
