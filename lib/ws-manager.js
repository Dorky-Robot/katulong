    ...
    switch (msg.type) {
      ...
      case "state-check":
        if (msg.clientId) {
          ...
          if (info && info.transport.readyState === 1) {
            info.transport.send(JSON.stringify({
      ...
      const wsMessageHandlers = {
        ...
        async pull() {
          ...
          if (data === null) {
            ...
            const chSnap = ch ? await ch.serializeScreen() : null;
            const snapshot = chSnap ?? (await session.serializeScreen() || "");
            transport.send(JSON.stringify({
              type: "pull-snapshot",
              session: name,
              data: snapshot,
