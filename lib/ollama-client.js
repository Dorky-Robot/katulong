...
export function createOllamaClient({
  ...
  // Dispatch on backend.kind. Default "direct" preserves the legacy
  // contract (POST /api/chat with NDJSON streaming). "queued" speaks
  // the queued backend protocol (POST /enqueue → poll GET /jobs/:hash).
  async function callBackend(backend, userPrompt, systemPrompt) {
    if (backend.kind === "queued") {
      return callQueuedOnce(backend, userPrompt, systemPrompt);
    }

    return callChatOnce(backend, userPrompt, systemPrompt);
  }
  ...
  /**
   * Speak the queued bridge protocol: POST /enqueue with a wrapped
   * { endpoint, body } payload, then poll GET /jobs/:hash until status
   * is "done" or "error".
   *
   * The aggregated response from the bridge mirrors what /api/chat with
   * `stream: false` would have returned, so result.message.content is
   * the assistant text — same final shape callChatOnce returns.
   *
   * Polling cadence: a short initial delay (so a fast cache hit on the
   * bridge returns ~immediately) ramping toward a steady poll interval.
   * Total budget is bounded by the same `timeoutMs` the streaming path
   * uses; past that we abort and the cascade falls through.
   */
  async function callQueuedOnce(backend, userPrompt, systemPrompt) {
    const messages = [];
    if (systemPrompt) messages.push({ role: "system", content: systemPrompt });
    messages.push({ role: "user", content: userPrompt });
    ...
    // Submit. The bridge forces stream:false on the upstream regardless
    // of what we send, but pass it through for parity / log clarity.
    const enqRes = await fetchImpl(`${backend.host}/enqueue`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        endpoint: "/api/chat",
        body: { model: backend.model, messages, stream: false },
      }),
      signal: AbortSignal.timeout(probeTimeoutMs * 3),
    });

    if (!enqRes.ok) {
      throw new Error(
        `queued backend enqueue ${enqRes.status}: ${await enqRes.text().catch(() => "(no body)")}`,
      );
    }
    ...
    // Hash format gate. queued backend emits sha256 hex (64 lowercase
    // hex chars). Validating shape before path-concatenating into the
    // poll URL prevents a hostile-or-buggy bridge from steering the
    // outbound `GET ${host}/jobs/...` to a different path via `../`,
    // URL-encoded slashes, etc.
    if (typeof hash !== "string" || !/^[a-f0-9]{64}$/.test(hash)) {
      throw new Error("queued backend enqueue response has malformed hash");
    }

    // Poll with a budget. Initial 250ms (catches dedup hits on a hot
    // bridge), ramp to 1s for steady-state generations.
    const deadline = Date.now() + timeoutMs;
    let interval = 250;

    for (;;) {
      if (Date.now() > deadline) {
        throw new Error(`queued backend poll timed out after ${timeoutMs}ms (hash=${hash})`);
      }

      await new Promise((resolve) => setTimeout(resolve, interval));
      interval = Math.min(interval + 250, 1_000);

      const pollRes = await fetchImpl(`${backend.host}/jobs/${hash}`, {
        headers: backend.authToken ? { Authorization: `Bearer ${backend.authToken}` } : {},
        signal: AbortSignal.timeout(probeTimeoutMs * 3),
      });

      if (pollRes.status === 404) {
        // Janitor swept the result before we could poll. Surface as a
        // retryable error so the cascade falls through to the next
        // backend or the next caller cycle.
        throw new Error(`queued backend job ${hash} not found (already swept?)`);
      }

      if (!pollRes.ok) {
        throw new Error(
          `queued backend poll ${pollRes.status}: ${await pollRes.text().catch(() => "(no body)")}`,
        );
      }
      ...
      if (job?.status === "error") {
        throw new Error(`queued backend job failed: ${job.error || "(no error message)"}`);
      }

      if (job?.status === "done") {
        const content = job?.result?.message?.content;
        if (typeof content !== "string" || content.length === 0) {
          throw new Error("queued backend job result missing message.content");
        }
        return content;
      }
  ...
  async function callOllama(userPrompt, { systemPrompt } = {}) {
    ...
    if (cachedBackend && now < cacheUntil) {
      try {
        return await callBackend(cachedBackend, userPrompt, systemPrompt);
      } catch (err) {
    ...
    for (const [i, backend] of backends.entries()) {
      ...
      try {
        const result = await callBackend(backend, userPrompt, systemPrompt);
