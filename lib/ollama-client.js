...
export function createOllamaClient({
  ...
  // the ollama-bridge protocol (POST /enqueue → poll GET /jobs/:hash).
  ...
  /**
   * Speak ollama-bridge's queue protocol: POST /enqueue with a wrapped
  ...
  async function callQueuedOnce(backend, userPrompt, systemPrompt) {
    ...
    if (!enqRes.ok) {
      throw new Error(
        `ollama-bridge enqueue ${enqRes.status}: ${await enqRes.text().catch(() => "(no body)")}`,
      );
    }
    ...
    // Hash format gate. ollama-bridge emits sha256 hex (64 lowercase
    ...
    if (typeof hash !== "string" || !/^[a-f0-9]{64}$/.test(hash)) {
      throw new Error("ollama-bridge enqueue response has malformed hash");
    }
    ...
    for (;;) {
      if (Date.now() > deadline) {
        throw new Error(`ollama-bridge poll timed out after ${timeoutMs}ms (hash=${hash})`);
      }
      ...
      if (pollRes.status === 404) {
        ...
        throw new Error(`ollama-bridge job ${hash} not found (already swept?)`);
      }

      if (!pollRes.ok) {
        throw new Error(
          `ollama-bridge poll ${pollRes.status}: ${await pollRes.text().catch(() => "(no body)")}`,
        );
      }
      ...
      if (job?.status === "error") {
        throw new Error(`ollama-bridge job failed: ${job.error || "(no error message)"}`);
      }

      if (job?.status === "done") {
        const content = job?.result?.message?.content;
        if (typeof content !== "string" || content.length === 0) {
          throw new Error("ollama-bridge job result missing message.content");
        }
        return content;
      }
