 ...
 * - LAN: Requires HTTPS (redirect to login on HTTP)
...
/**
 * Paths that are explicitly allowed over HTTP.
 * Previously used for CA certificate installation; now empty since trust flow is removed.
 */
...
export function checkHttpsEnforcement(req, pathname, isPublicPath, isHttpsConnection) {
  ...
  if (isPublicPath(pathname)) {
    // LAN: Require HTTPS even for public paths
    // This prevents accessing login page over HTTP on LAN
    if (accessMethod === "lan") {
      // All paths on LAN require HTTPS
  ...
  const redirectTarget = getHttpsRedirectTarget(accessMethod);
 ...
 */
function getHttpsRedirectTarget(accessMethod) {
  // Both LAN and internet redirect to login page
  return "/login";
}
...
export function getUnauthenticatedRedirect(req) {
  // All unauthenticated users redirect to login
  return "/login";
}
