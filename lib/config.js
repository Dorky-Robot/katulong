...
/**
 * Minimal TOML reader for the `[[host]]` array-of-tables shape used by
 * `~/.sipag/hosts.toml`. Recognized keys per host: id, url, apiKey,
 * label. Anything else is ignored. Returns [] on any parse error so
 * callers can fall back gracefully — this is auto-import bootstrap, not
 * a config promise.
 *
 * Exported for unit testing — `_readSipagHostsFallback` swallows errors
 * which makes regression coverage hard to write at the integration
 * layer.
 */
export function parseSipagHostsToml(text) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    // Bare top-level scalars (anything before first [[host]]) are
    // ignored — the file may grow other sections later.
    if (!current) continue;
    const m = line.match(/^([a-zA-Z][a-zA-Z0-9_]*)\s*=\s*"((?:[^"\\]|\\.)*)"\s*$/);
    if (!m) continue;
    current[m[1]] = m[2].replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }
  if (current) peers.push(current);
  // Filter to records that have the minimum viable shape — id, url, apiKey.
  return peers.filter(
...
export class ConfigManager {
  ...
  async setSipagUrl(value) {
    ...
    if (trimmed.startsWith("/")) {
      // Reject protocol-relative URLs (`//host/path`). They start
      // with `/` and pass the no-`..` check, but the browser resolves
      // them as cross-origin URLs against the page's protocol — a
      // sipag tile assigning such a value to iframe.src would point
      // at attacker-controlled `host` with the sandbox's
      // `allow-top-navigation-by-user-activation` letting a single
      // user tap phish the top window.
      if (trimmed.startsWith("//")) {
        throw new Error("sipagUrl must not be protocol-relative (start with //)");
      }
      // Reject path traversal up the tree; keep the value confined
      // to a recognizable shape so a buggy setter can't end up
      // serving arbitrary cross-host content via the same field.
      if (trimmed.includes("..")) {
        throw new Error("sipagUrl path must not contain ..");
      }
      // Length cap matches the absolute-URL setter rigor — defends
      // against a buggy caller stuffing arbitrary bytes through here.
      if (trimmed.length > 2048) {
        throw new Error("sipagUrl must be 2048 characters or less");
      }
      // Reject control characters; they have no business in URLs and
      // could confuse downstream serializers / SSRF guards.
      // eslint-disable-next-line no-control-regex
      if (/[\u0000-\u001f\u007f]/.test(trimmed)) {
        throw new Error("sipagUrl must not contain control characters");
      }
      await this.withLock(() => this._set('sipagUrl', trimmed));
      return;
    }
    // Length cap mirroring the relative-path branch — defense-in-depth
    // against a buggy caller stuffing arbitrary bytes through here.
    if (trimmed.length > 2048) {
      throw new Error("sipagUrl must be 2048 characters or less");
    }
  ...
  getConfig() {
    if (!this.config) this.initialize();
    const { ollamaPeerToken, peers, ...safe } = this.config;
    // Strip apiKey from each peer record. The picker UI calls
    // /api/peers/:id/credentials when it actually needs the key
    // (auth-gated, separate route) — bulk dumps go through here.
    if (Array.isArray(peers)) {
      safe.peers = peers.map(({ apiKey: _drop, ...rest }) => rest);
    }
    return safe;
  }

  /**
   * List configured peer katulong instances. Each entry is the public
   * shape: `{ id, url, label }`. The api key never leaves this module
   * via the list path — the caller in peers-routes pulls it from
   * `getPeerById` only when it needs to make an authenticated outbound
   * call.
   *
   * If no peers are configured but `~/.sipag/hosts.toml` exists, this
   * returns peers derived from that file as a one-shot bootstrap. The
   * derived list is NOT persisted — once the user runs an explicit
   * `setPeers`, sipag and katulong configs decouple. The auto-import
   * path keeps the spike usable without a separate provisioning step.
   */
  getPeers() {
    const stored = this._get("peers", null);
    // Once `peers` has been explicitly set (even to []), we return what
    // the user chose — never fall back to the sipag hosts.toml import.
    // The fallback ONLY fires when the field has never been written
    // (initial bootstrap) so an explicit "I want no peers" can't be
    // silently overruled by the auto-import. `Array.isArray(null)` is
    // false, which is the discriminator.
    if (Array.isArray(stored)) {
      return stored.map(({ id, url, label }) => ({ id, url, label: label || id }));
    }
    const fromSipag = this._readSipagHostsFallback();
    return fromSipag.map(({ id, url, label }) => ({ id, url, label: label || id }));
  }

  /**
   * Look up a single peer by id and return the full record including
   * apiKey. Used by the cross-instance proxy + credentials routes.
   * Returns null if not found.
   *
   * NEVER expose this return value via a `getConfig()`-style endpoint —
   * the apiKey is a server-side cross-instance credential.
   */
  getPeerById(id) {
    if (!id) return null;
    const stored = this._get("peers", null);
    // Same null-vs-array distinction as getPeers — once the user has
    // written ANY explicit peers list, that is the source of truth,
    // even if it's empty.
    const list = Array.isArray(stored) ? stored : this._readSipagHostsFallback();
    return list.find((p) => p.id === id) || null;
  }

  /**
   * Persist a peers array. Each entry must have:
   *   id     — short identifier (used in URLs, filenames-safe)
   *   url    — http(s)://host[:port] of the peer katulong
   *   apiKey — peer's api key (we send it as Bearer)
   *   label  — optional human label (defaults to id)
   *
   * Throws on validation failure rather than silently dropping bad
   * entries — silent partial saves are how stale config rots.
   */
  async setPeers(arr) {
    if (arr === null) {
      // Store an explicit empty array, not null. `null` means "never
      // set, fall back to ~/.sipag/hosts.toml" — that's reserved for
      // first-boot bootstrap. A user calling setPeers(null) wants
      // "remove my peers", which must be honored verbatim.
      await this.withLock(() => this._set("peers", []));
      return;
    }
    if (!Array.isArray(arr)) {
      throw new Error("peers must be an array or null");
    }
    const normalized = [];
    const seen = new Set();
    for (const raw of arr) {
      if (!raw || typeof raw !== "object") {
        throw new Error("each peer must be an object");
      }
      ...
      if (seen.has(id)) {
        throw new Error(`duplicate peer id: ${id}`);
      }
      seen.add(id);
      let parsed;
      try { parsed = new URL(url); } catch { throw new Error(`peer ${id}: url must be a valid URL`); }
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        throw new Error(`peer ${id}: url must use http or https`);
      }
      if (parsed.username || parsed.password) {
        throw new Error(`peer ${id}: url must not contain credentials`);
      }
      if (apiKey.length < 16 || apiKey.length > 512) {
        throw new Error(`peer ${id}: apiKey must be 16-512 characters`);
      }
      normalized.push({
        id,
        url: url.replace(/\/+$/, ""),
    ...
    await this.withLock(() => this._set("peers", normalized));
  }

  /**
   * Best-effort read of `~/.sipag/hosts.toml`. Returns an array of
   * peer records (with apiKey) on success, [] otherwise. Pure helper —
   * does not cache, does not write, does not throw on missing file.
   *
   * The format we accept is the minimal `[[host]]` array-of-tables:
   *   [[host]]
   *   id     = "mini"
   *   url    = "https://katulong-mini.felixflor.es"
   *   apiKey = "…"
   *
   * We deliberately don't pull in a TOML library — the parser here is
   * just enough for that one shape. The day someone wants nested tables
   * or arrays-of-strings is the day to swap in `@iarna/toml`.
   */
  _readSipagHostsFallback() {
    try {
      const home = process.env.HOME;
      if (!home) return [];
      const path = join(home, ".sipag", "hosts.toml");
      if (!existsSync(path)) return [];
      const text = readFileSync(path, "utf-8");
      return parseSipagHostsToml(text);
    } catch {
      return [];
    }
  }
