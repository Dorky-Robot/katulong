...
export class ConfigManager {
  ...
  /**
   * Get the wire protocol the configured peer-bridge speaks.
   *
   *   "direct"  — opaque pass-through; calls go to /api/chat with NDJSON
   *               streaming (same shape as ollama serve). Default.
   *   "queued"  — POST /enqueue → poll GET /jobs/:hash, returns one
   *               aggregated result. Picks up callers that point at a
   *               queue-fronted Ollama instead of the daemon directly.
   *
   * Per-instance so a host can pick the kind that matches whatever it's
   * actually pointing at.
   */
  getOllamaPeerKind() {
    const raw = this._get('ollamaPeerKind', 'direct');
    return raw === 'queued' ? 'queued' : 'direct';
  }

  async setOllamaPeerKind(value) {
    if (value !== 'direct' && value !== 'queued') {
      throw new Error("ollamaPeerKind must be 'direct' or 'queued'");
    }
    await this.withLock(() => this._set('ollamaPeerKind', value));
  }
