...
export class ConfigManager {
   ...
   *
   *   "direct"  — legacy katulong-bridge (opaque pass-through; calls go
   *               to /api/chat with NDJSON streaming). Default.
   *   "queued"  — ollama-bridge (POST /enqueue → poll GET /jobs/:hash).
   *
   * The choice is per-instance because a host might still point at the
   * legacy bridge during the migration window.
   */
  getOllamaPeerKind() {
