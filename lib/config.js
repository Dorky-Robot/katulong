  ...
  initialize() {
    if (existsSync(this.configPath)) {
      try {
        ...
        // Ensure instanceIcon exists and is valid (for backward compatibility and defense-in-depth)
        if (!this.config.instanceIcon || !/^[a-z0-9-]+$/.test(this.config.instanceIcon)) {
          if (this.config.instanceIcon && !/^[a-z0-9-]+$/.test(this.config.instanceIcon)) {
            log.warn("Invalid instanceIcon in config, resetting to default", { value: this.config.instanceIcon });
          }
