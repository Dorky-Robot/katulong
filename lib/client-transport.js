...
export function createClientTransport(ws) {
  ...
  function wireDcEvents(dataChannel) {
    dataChannel.onmessage = (event) => {
      if (activeType !== "datachannel") return;
      // node-datachannel polyfill may pass a MessageEvent (with .data)
      // or the raw data directly — handle both.
  ...
  return {
    ...
    /** readyState of the active transport (always numeric: 0-3). */
    get readyState() {
      if (activeType === "datachannel" && dc) {
        // node-datachannel uses string states ("connecting", "open", "closing", "closed")
        // while WebSocket uses numeric (0-3). Normalize to numeric.
        const s = dc.readyState;
        if (typeof s === "number") return s;
        const map = { connecting: 0, open: 1, closing: 2, closed: 3 };
        return map[s] ?? 3;
      }
