...
export async function processRegistration({
  ...
  try {
    ...
    const updatedState = state
      ...
      .addSession(session.token, session.expiry, enrichedCred.id, session.csrfToken);
...
export async function processAuthentication({
  ...
  try {
    ...
    const updatedState = currentState
      .pruneExpired()
      .addSession(session.token, session.expiry, storedCred.id, session.csrfToken);
...
export function processPairing({ pairingResult, currentState, deviceId, deviceName, userAgent }) {
  ...
  const updatedState = state
    ...
    .addSession(session.token, session.expiry, credentialId, session.csrfToken);
