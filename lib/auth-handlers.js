...
import { verifyRegistration, verifyAuth, createLoginToken } from "./auth.js";
...
export async function processRegistration({
  ...
  try {
    ...
    const updatedState = state
      ...
      .addLoginToken(session.token, session.expiry, enrichedCred.id, session.csrfToken, session.lastActivityAt);
...
export async function processAuthentication({
  ...
  try {
    ...
    const updatedState = currentState
      ...
      .addLoginToken(session.token, session.expiry, storedCred.id, session.csrfToken, session.lastActivityAt);
