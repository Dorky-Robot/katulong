...
export function createTopicBroker({ pubsubDir } = {}) {
   ...
   *
   * `meta.timestamp` (ms since epoch) lets the caller stamp the event with
   * the time the underlying fact actually happened (e.g. the Claude
   * transcript entry's timestamp) rather than "now". The feed renderer sorts
   * by this field, so pulling from a transcript written minutes ago now
   * interleaves correctly with live events. Falls back to `Date.now()` when
  ...
  function publish(topic, message, meta = {}) {
    ...
    const callerTs = typeof meta.timestamp === "number" && Number.isFinite(meta.timestamp)
      ? meta.timestamp
      : null;
