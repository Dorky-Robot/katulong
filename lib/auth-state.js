...
export class AuthState {
  ...
  migrateCredentialMetadata(now = Date.now()) {
    ...
    return {
      state: new AuthState({ user: this.user, credentials, sessions: this.sessions, setupTokens: this.setupTokens }),
      migrated: true,
    };
  }

  /**
   * Remove orphaned sessions — old format, missing credentialId, or dangling references (immutable)
  ...
  cleanOrphanedSessions() {
    ...
    const sessions = Object.fromEntries(
      Object.entries(this.sessions).filter(([_, session]) => {
        if (typeof session === 'number') { cleaned = true; return false; }
        if (!('credentialId' in session)) { cleaned = true; return false; }
        if (session.credentialId === null) { cleaned = true; return false; }
        if (!validCredentialIds.has(session.credentialId)) { cleaned = true; return false; }
        return true;
      })
    );
    if (!cleaned) return { state: this, cleaned: false };
    return {
      state: new AuthState({ user: this.user, credentials: this.credentials, sessions, setupTokens: this.setupTokens }),
      cleaned: true,
    };
  }

  /**
   * Add lastActivityAt to sessions that don't have it (immutable)
   * @param {number} now - Current timestamp
   * @returns {{ state: AuthState, migrated: boolean }}
   */
  migrateSessionActivity(now = Date.now()) {
    let migrated = false;
    const sessions = Object.fromEntries(
      Object.entries(this.sessions).map(([token, session]) => {
        if (session.lastActivityAt) return [token, session];
        migrated = true;
