...
export class AuthState {
   ...
   */
  addSession(token, expiry, credentialId = null) {
    return new AuthState({
      user: this.user,
      credentials: this.credentials,
      sessions: { ...this.sessions, [token]: { expiry, credentialId } },
    });
  }
  ...
  isValidSession(token, now = Date.now()) {
    ...
    // Whitelist validation: If session has credentialId (not null), verify credential still exists
    // Pairing sessions (credentialId: null) are exempt from credential validation
