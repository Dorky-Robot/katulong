...
/** Setup tokens expire after 1 hour to limit the window for leaked tokens. */
...
export class AuthState {
   ...
   * Returns null if the token is not found or has expired (TTL: SETUP_TOKEN_TTL_MS).
  ...
  findSetupToken(tokenValue, now = Date.now()) {
    ...
    // Check expiration after the full constant-time scan (expiry is metadata, not secret)
    if (found !== null && found.createdAt && (now - found.createdAt) >= SETUP_TOKEN_TTL_MS) {
      return null;
    }
    return found;
  }
