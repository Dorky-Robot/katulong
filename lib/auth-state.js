...
export class AuthState {
   ...
   */
  constructor({ user, credentials, sessions, loginTokens, setupTokens, apiKeys }) {
    ...
    this.apiKeys = Object.freeze((apiKeys || []).map(k => Object.freeze({ ...k })));
    Object.freeze(this);
  }
  ...
  removeApiKey(id) {
    return this._with({ apiKeys: this.apiKeys.filter(k => k.id !== id) });
  }
  ...
  findApiKey(keyValue) {
    ...
    for (const k of this.apiKeys) {
      if (k.hash && k.salt && verifyToken(keyValue, k)) {
        found = k;
      }
    }
  ...
  static fromJSON(data) {
    ...
    const state = new AuthState({
      ...
      loginTokens: data.loginTokens || data.sessions || {},
      setupTokens,
