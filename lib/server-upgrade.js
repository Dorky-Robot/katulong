...
export function createUpgradeHandler({
  ...
  function validateUpgradeOrigin(req) {
    ...
    // API-key auth is the explicit cross-instance grant — the user
    // minted the key and pasted it into a peer katulong's hosts.toml
    // (or sipag's keys/), which is itself the consent gesture. Origin
    // checks exist to stop a cookie-bearing user being CSRF'd from
    // some random page; they don't apply when auth comes from a
    // bearer-style key the attacker would already need to possess.
    if (req._apiKeyAuth) return true;
