...
log "  katulong update"
log ""
log "(\`katulong update\` sets the .update-in-progress sentinel that the formula's"
log "post_install honors, so the running service swaps over without losing port 3001."
log "Plain \`brew upgrade dorky-robot/tap/katulong\` triggers post_install's own"
log "service restart, which can race the still-listening old server and leave the"
log "LaunchAgent down — KeepAlive=SuccessfulExit=false won't auto-respawn it.)"
