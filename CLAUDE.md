...
Katulong is a self-hosted web terminal that gives remote shell access to the host machine over HTTP + WebSocket. It multiplexes PTY sessions via a Unix socket daemon and serves a single-page xterm.js frontend. Remote access is provided via tunnel services (cloudflared, ngrok) rather than direct LAN exposure.

## Architecture

- `server.js` — HTTP + WebSocket server (routes, auth middleware, daemon IPC); listens on localhost only
...
- `lib/p2p.js` — WebRTC DataChannel for low-latency terminal I/O (works via tunnel signaling)
...
- **Port conflict (EADDRINUSE)**: Stop background servers before pushing: `lsof -ti:3001 | xargs kill -9`
- **E2E timeout**: Ensure no dev servers are running on test ports (3001)
...
### Authentication
- First device registers via WebAuthn (passkey) through the tunnel URL. All subsequent devices also use WebAuthn registration.
- Localhost requests (`127.0.0.1`, `::1`) bypass auth (auto-authenticated). Remote access uses tunnel services (cloudflared, ngrok).
- Remote requests require a valid `katulong_session` cookie.
...
- **TLS**: Handled by the tunnel provider (cloudflared, ngrok). The server listens on HTTP localhost only. Only actual TLS socket state (`req.socket.encrypted`) is trusted for setting Secure cookie flag; tunnel-specific headers (CF-Visitor, hostname patterns) are used as secondary signals.
...
4. **Input handling**: Server-side code must never pass unsanitized input to `child_process`, `exec`, or similar. Validate all input formats.
...
10. **File I/O**: Auth state writes must be atomic (temp + rename). Add error handling for corrupt JSON.
11. **Environment variables**: Never expose sensitive env vars (SSH_PASSWORD, SETUP_TOKEN, KATULONG_NO_AUTH) to PTY processes.
12. **Header trust**: Never trust request headers (`X-Forwarded-*`) for security decisions. Only trust actual socket state. Tunnel-specific headers (CF-Visitor, CF-Connecting-IP) may be used as secondary signals for HTTPS detection only.
13. **State mutations**: All auth state mutations must use `withStateLock()` from `lib/auth.js` to prevent race conditions.
