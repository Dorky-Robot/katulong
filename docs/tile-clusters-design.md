...
- [ ] **MC1e — Persistence to `uiStore` + session identity fix.** Two changes that both require a localStorage schema bump, combined so there is exactly one migration rather than two.
  ...
  **Also in scope — belt-and-braces for the rename class.**
  - Extract `renameSession(id, newDisplayName)` as the single entry point that dispatches the store action; `tab-rename` event handler, `session-list-component`, and WS `session-updated` all call it. The manually-enumerated fan-out in `app.js`'s `applyLocalRename` goes away once per-store renames are no-ops (stores key by id).
  - Add `test/session-rename-entry-points.test.js`: each entry point (inline tab-rename, session-list modal, Option+R, WS echo) must leave exactly one server-side tmux session afterward. This is the test the post-mortem in `docs/session-identity.md` says would have caught MC1c's regression at review time.

  **Sequencing.** Land after MC1d (which removes the last carousel reads from app.js). MC1d is a prerequisite because the rename reducer lives in the ui-store layer that MC1d finishes wiring up. Pre-MC3 is strongly preferred — L2 strips will key off sessions and dragging them through another key-migration later compounds the risk.
