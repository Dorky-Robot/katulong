...
- [x] **MC3 — Level 2 cluster strips** (MVP). Single-PR landing covering: `level: 1|2` state on `uiStore` (with `SET_LEVEL` + `setLevel()`), fixed overlay `#cluster-strips` as a pure projection of uiStore, tap-on-strip → `switchCluster` + return to L1, tap-on-card → also `focusTile`, `+` at L2 → `addCluster({ switchTo: false })` (new strip stays in L2), L1↔L2 pinch wiring (reuses `pinch-gesture.js` — touch + trackpad `wheel+ctrlKey`). **Deferred to follow-ups:** axonometric tilted decks (columns are single-row today; flat cards suffice until drag-to-stack lands), animated L1↔L2 DOM morph (live terminal DOM stays mounted under the overlay in `#terminal-container`, no teardown needed), per-column focused-row state (no multi-row columns yet). 12 new tests (`test/cluster-strips.test.js`) + 4 new `SET_LEVEL` tests + 11 `pinch-gesture` tests. *Medium.*
