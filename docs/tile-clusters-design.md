...
- [x] **MC1c — shortcut-bar as `uiStore` projection.** The `<tile-tab-bar>` custom element already subscribes to `uiStore` directly; MC1c closes it out by deleting the legacy carousel-reading code path in `public/lib/shortcut-bar.js` (1158 → ~290 lines). Module now owns only the surrounding chrome — `+` button dropdown, key-island tool row, and a generic menu helper for host-initiated popups (e.g. tab-context-menu events from `<tile-tab-bar>`). Removed public methods `setActiveTab` and `renameTabEl` (the web component self-updates via subscription); removed options `onSessionClick`, `onTabRenamed`, `onAllTabsClosed`, `getInstanceIcon`, `getSessionIcon`, `carousel`. Deleted unreachable `shortcut-bar-desktop.js` / `-ipad.js` / `-phone.js`. WS `session-renamed` effect pipeline dropped `shortcutBarRename` in favor of `iconStoreRename` + the existing `uiStoreRename`. Carousel exports that only shortcut-bar consumed are now unreferenced and can be deleted in MC1d alongside app.js's own shrinkage.

- [x] **MC1d — app.js as `uiStore` projection.** Dropped the remaining `carousel.isActive()` state reads in `public/app.js` — all five call sites (`subscribeAll` refit, `fitActiveTerminal`, `applyLocalRename`, drag-drop `shouldIgnore`, WS `carouselRename` effect) either lean on carousel's own idempotent guards (`fitAll` / `renameCard` are no-ops when inactive or when the id isn't in `cards`) or swap to `uiStore.getState().focusedId` as the "do we have tiles" signal. Extracted `parseLegacyCarouselStorage()` as a module-level export from `card-carousel.js` so `gatherBootState()` migrates the old `katulong-carousel` blob without reaching into a carousel instance; the instance's `restore()` now delegates to the same parser. After this lands, every carousel call in app.js is an imperative command (`fitAll`, `renameCard`) — no state reads. `getCards` / `getFocusedCard` / `getTile` / `findCard` / `isActive` are still used by `tile-host` (the one legitimate consumer of carousel state) and by carousel's own tests, so they stay for now. *Medium.*

- [ ] **MC1e — Persistence to `uiStore` + session identity fix.** Two changes that both require a localStorage schema bump, combined so there is exactly one migration rather than two.

  **Part 1 — Persistence move.** Relocate `save`/`restore` from `card-carousel.js` into the ui-store layer. `MAX_PERSISTED_CARDS = 50` becomes a per-cluster cap (falls out naturally once each cluster's tiles are persisted from `clusters[c]`). Schema bumps to the workspace shape from "Architectural decisions" §3: `{ clusters: [{ id, columns, ... }], activeClusterId }`.

  **Part 2 — Surrogate session `id`.** See `docs/session-identity.md` for the full root-cause write-up. Session display names are currently a mutable primary key across 15+ stores (server `sessions` map, `subscriptions` map, client tracker, output coalescer, tmux itself, `terminalPool`, `carousel.cards`, `notepad`, `windowTabSet`, `uiStore.tiles`, shortcut bar DOM, URL `?s=`, `document.title`, `localStorage`, `BroadcastChannel`). Every rename is a hand-transcribed fan-out across all of them, and every new entry point re-enumerates the fan-out — four separate rename bugs shipped in six weeks (`35fb493`, `834f6d1`, `a4c7940`, `b0c7bf3`), plus the MC1c follow-up fix. The structural cure: give every session an immutable `id` at creation; `displayName` becomes a field. All stores key by `id`. `tmuxName` is derived from `id` (`kat_<id>`), never changes, so the `tmuxSessionName` sanitization function in `lib/tmux.js:15` and its conflict-check caller in `lib/session-manager.js:62` both become dead code and are deleted in the same PR. URL carries `?s=<id>`; backward-compat for existing `?s=<name>` bookmarks is handled by a **server-side redirect** — `GET /?s=<name>` looks up the session by name and 302s to `?s=<id>`. Keeping the resolver on the server preserves the existing boundary (server owns session lookup) rather than adding a new client-side query endpoint. WebSocket `session-renamed` becomes `session-updated { id, displayName }` — a one-field update, not a key migration. The WS protocol has no backward-compat obligation: a connected client is always the same deployed version as the server.

  **Why combine the two.** Part 1 already bumps the persistence schema and rewrites every client-side store that currently keys by session name. Part 2 needs the same stores to re-key. Doing them separately means two migrations and two rounds of store rewrites; doing them together means the migration reducer reads old `name`-keyed state once, issues a fresh `id` per terminal tile, and writes out the new shape. The v3 → v4 migration becomes load-bearing (name → id), not just a shape bump.

  **Also in scope — belt-and-braces for the rename class.**
  - **Dissolves a latent tile-host bug.** `tile-host.reconcileTiles` keys its `handles` Map by id. The current rename pipeline (`session-renamed` effects) invokes `carousel.renameCard(oldName, newName)` before `uiStoreRename` dispatches, so by the time reconcile runs, `uiStore.tiles` contains the new id but carousel's DOM already has a card under the new id. Reconcile's remove-then-add path then short-circuits in `carousel.addCard` (card already present) and the `handles.set(id, adapter._handle)` guard at `public/lib/tile-host.js:123` is skipped because `adapter._handle` is null — so the handle for the renamed tile is orphaned until the next full remount. MC1e's surrogate-id move eliminates this entirely: id never changes on rename, `reconcileTiles` sees the same key throughout, and `carousel.renameCard` is replaced by a `setTitle` on the existing handle. Flagged by correctness review of PR #589; orthogonal to MC1d so not fixed there.
  - Extract `renameSession(id, newDisplayName)` as the single entry point that dispatches the store action; `tab-rename` event handler, `session-list-component`, and WS `session-updated` all call it. The manually-enumerated fan-out in `app.js`'s `applyLocalRename` goes away once per-store renames are no-ops (stores key by id). **Transitional**: once the per-store name-keyed stores are fully retired inside this same PR, the helper is a thin wrapper around one dispatch call — delete it in the same PR and have callers dispatch directly. The helper exists only to keep the in-flight migration's call-site edits small; it should not survive the PR.
  - Split regression coverage across the two layers already supported by `test/`:
    - **Unit**: assert the `tab-rename` CustomEvent handler calls `api.put` (mock `api`, assert call). No DOM automation, runs under `npm run test:unit`.
    - **Integration**: using the existing `test/helpers/session-manager-fixture.js`, call `PUT /sessions/:id` with `{ displayName }` and assert exactly one tmux session exists afterward with the updated display name. Runs under `npm run test:integration`.
    - Avoid a browser-automation test for this — the existing harness has no browser runner and adding one for one test is the wrong trade.
...
- [ ] **MC1f — Session meta bucket.** See `docs/session-meta.md` for the full write-up. Adds `session.meta` — a freeform, namespaced (`claude` / `user` / `system`), size-capped (4 KB) per-session key/value store that survives rename (it hangs off `id`, not `name`). Persisted to `sessions.json` under `meta` alongside `tmuxName`/`id`, minus the `claude` namespace which is reconstructed from live hook events. New routes: `GET/PATCH /sessions/:id/meta`, `DELETE /sessions/:id/meta/:ns[/:key]`, all `auth + csrf`. CLI: `katulong session meta <name> [set|unset] <path>=<value>`; `session list --wide` grows a `CLAUDE` column.

  **First consumer — Claude presence.** Closes the tile → claude feed-button gap documented in `docs/tile-claude-session-link.md` without re-coupling anything to mutable session name. `relay-hook` stamps `_tmuxPane` on the hook payload; server looks up the session by pane (captured on Session in MC1e PR2 — see below), writes `meta.claude = { uuid, since }` on `SessionStart`, clears it on `Stop` / `SessionEnd`. The frontend feed button then reads `session.meta.claude.uuid` directly; no more `sessionName === activeSessionName` fallback. Future consumers can tag their own data (`meta.user.tags`, `meta.system.autoRestart`) without adding new Session fields.

  **Sequencing.** Land after **MC1e PR2** (which adds `tmuxName = kat_<id>` and captures `$TMUX_PANE` on Session). The pane capture is the key MC1f writes through; without it, no hook-ingest handler can answer "which session is this?" from a `SessionStart` event. Split into two PRs: PR1 = `session.meta` plumbing (Session field, persistence, routes, CLI, caps); PR2 = claude hook → `meta.claude` wiring (relay-hook pane stamping, `SessionStart`/`SessionEnd` subscription in `setup.js`, ingest handler writes meta). PR2 is small and isolated once PR1 lands. *Medium — the cap + namespace discipline is the subtlety; the data shape is trivial.*

  **Not in scope.** Query-by-meta endpoint, a meta history log, a `session-meta-updated` WS message, and binary/large-blob storage are all deferred. The first consumer (Claude tile tools) only needs per-session lookup; broader queries come back if a real consumer asks for them.
