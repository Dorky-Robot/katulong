 ...
 *
 * This test pins parity directly: the same `ScreenState.term` xterm.js
 * Terminal goes through BOTH implementations and the hashes must agree
 * byte-for-byte. If a future change breaks parity, this test fails
 * immediately and points at the divergent file.
 */
...
/**
 * Build a Session and write some content into its ScreenState mirror so
 * the buffer has a non-trivial, deterministic state to fingerprint.
 *
 * The session is NOT attached to a real tmux process — it's only used as
 * a host for the screen mirror and to expose `screenFingerprint()`. The
...
async function makeFingerprintableSession(content) {
  ...
  // Write directly to the underlying xterm Terminal (bypassing tmux protocol).
  ...
  await new Promise(resolve => session._screen.term.write(content, resolve));
  return session;
}
...
describe("client/server screenFingerprint parity", () => {
  it("hashes match for an empty terminal", async () => {
    ...
    const client = clientFingerprint(session._screen.term);
  ...
  it("hashes match after a resize (different cols/rows)", async () => {
    ...
    // both sides. The session resizes its ScreenState via `_applyResize`,
    // but here we call `_screen.resize` directly to skip tmux side-effects.
    const session = await makeFingerprintableSession("padded text\r\n");
    session._screen.resize(120, 30);
    // Flush the resize through xterm's internal queue.
    await new Promise(resolve => session._screen.term.write("", resolve));
    const server = await session.screenFingerprint();
    const client = clientFingerprint(session._screen.term);
    assert.strictEqual(server.hash, client,
  ...
  it("hashes change when content changes (sanity check)", async () => {
    ...
    const clientA = clientFingerprint(sessionA._screen.term);
    const clientB = clientFingerprint(sessionB._screen.term);
