...
describe("AuthState", () => {
  ...
  describe("setup token hashing", () => {
    ...
    describe("addSetupToken", () => {
      it("stores hash and salt instead of plaintext token", () => {
        const state = AuthState.empty("user123").addSetupToken(makeTokenData());
        const stored = state.setupTokens[0];

        assert.ok(stored.hash, "stored entry should have a hash field");
        assert.ok(stored.salt, "stored entry should have a salt field");
        assert.strictEqual(stored.token, undefined, "plaintext token must not be stored");
      });

      it("hash and salt are hex strings of expected length", () => {
        const state = AuthState.empty("user123").addSetupToken(makeTokenData());
        const stored = state.setupTokens[0];

        // salt = 16 bytes = 32 hex chars
        assert.strictEqual(stored.salt.length, 32);
        // hash = 64 bytes (scrypt keyLen) = 128 hex chars
        assert.strictEqual(stored.hash.length, 128);
      });

      it("two calls with the same plaintext produce different hashes (different salts)", () => {
        const base = AuthState.empty("user123");
        const s1 = base.addSetupToken(makeTokenData({ id: "t1" }));
        const s2 = base.addSetupToken(makeTokenData({ id: "t2" }));

        assert.notStrictEqual(s1.setupTokens[0].hash, s2.setupTokens[0].hash);
        assert.notStrictEqual(s1.setupTokens[0].salt, s2.setupTokens[0].salt);
      });

      it("preserves metadata fields (id, name, createdAt, expiresAt)", () => {
        const td = makeTokenData();
        const state = AuthState.empty("user123").addSetupToken(td);
        const stored = state.setupTokens[0];

        assert.strictEqual(stored.id, td.id);
        assert.strictEqual(stored.name, td.name);
        assert.strictEqual(stored.createdAt, td.createdAt);
        assert.strictEqual(stored.expiresAt, td.expiresAt);
      });
    });

    describe("findSetupToken (hash/validate round-trip)", () => {
      it("finds a valid token by its plaintext value", () => {
        const td = makeTokenData();
        const state = AuthState.empty("user123").addSetupToken(td);

        const found = state.findSetupToken("plaintext-token-value", Date.now());
        assert.ok(found, "should find the token");
        assert.strictEqual(found.id, "tok1");
      });

      it("returns null for a wrong token value", () => {
        const state = AuthState.empty("user123").addSetupToken(makeTokenData());

        const found = state.findSetupToken("wrong-value", Date.now());
      ...
      it("returns null for an empty string token", () => {
        ...
        assert.strictEqual(state.findSetupToken("", Date.now()), null);
        assert.strictEqual(state.findSetupToken(null, Date.now()), null);
        assert.strictEqual(state.findSetupToken(undefined, Date.now()), null);
      });

      it("rejects an expired token even when the hash matches", () => {
        const td = makeTokenData({ expiresAt: Date.now() - 1000 }); // already expired
        const state = AuthState.empty("user123").addSetupToken(td);

        const found = state.findSetupToken("plaintext-token-value", Date.now());
        assert.strictEqual(found, null, "expired token must be rejected");
      });

      it("rejects a token exactly at its expiry boundary", () => {
        const expiresAt = 1_000_000;
        const td = makeTokenData({ expiresAt });
        const state = AuthState.empty("user123").addSetupToken(td);

        // now === expiresAt => expired (fail-closed: now >= expiresAt)
        assert.strictEqual(state.findSetupToken("plaintext-token-value", expiresAt), null);
      });
      ...
      it("accepts a token one millisecond before expiry", () => {
        ...
        const found = state.findSetupToken("plaintext-token-value", expiresAt - 1);
        assert.ok(found, "token should still be valid one ms before expiry");
      });

      it("rejects a token with no expiresAt (fail-closed)", () => {
        const td = makeTokenData({ expiresAt: undefined });
        const state = AuthState.empty("user123").addSetupToken(td);

        const found = state.findSetupToken("plaintext-token-value", Date.now());
        assert.strictEqual(found, null, "token without expiresAt must be rejected");
      });

      it("does not short-circuit — iterates all tokens even after a match", () => {
        // Three tokens; the matching one is first. All three must be iterated.
        const base = AuthState.empty("user123");
        const state = base
          .addSetupToken(makeTokenData({ id: "t1", token: "match-me" }))
          .addSetupToken(makeTokenData({ id: "t2", token: "other-1" }))
          .addSetupToken(makeTokenData({ id: "t3", token: "other-2" }));

        const found = state.findSetupToken("match-me", Date.now());
        assert.ok(found);
        assert.strictEqual(found.id, "t1");
      });

      it("finds the correct token among many", () => {
        const base = AuthState.empty("user123");
        const state = base
          .addSetupToken(makeTokenData({ id: "t1", token: "aaa" }))
          .addSetupToken(makeTokenData({ id: "t2", token: "bbb" }))
          .addSetupToken(makeTokenData({ id: "t3", token: "ccc" }));

        assert.strictEqual(state.findSetupToken("aaa", Date.now()).id, "t1");
        assert.strictEqual(state.findSetupToken("bbb", Date.now()).id, "t2");
        assert.strictEqual(state.findSetupToken("ccc", Date.now()).id, "t3");
        assert.strictEqual(state.findSetupToken("ddd", Date.now()), null);
      });
    });

    describe("fromJSON migration — legacy plaintext tokens", () => {
      it("migrates a plaintext token entry to hashed format on load", () => {
        const data = {
          user: { id: "user123", name: "owner" },
          credentials: [],
          sessions: {},
        ...
        assert.ok(stored.hash, "migrated entry should have a hash");
        assert.ok(stored.salt, "migrated entry should have a salt");
        assert.strictEqual(stored.token, undefined, "plaintext must be removed");
        assert.strictEqual(stored.id, "legacy1");
        assert.strictEqual(stored.name, "Legacy");
      });

      it("migrated token is usable — findSetupToken succeeds with the original plaintext", () => {
        ...
        const state = AuthState.fromJSON(data);
        const found = state.findSetupToken("legacy-plaintext", Date.now());

        assert.ok(found, "should find the migrated token using the original plaintext");
        assert.strictEqual(found.id, "legacy1");
      });

      it("migrated token rejects wrong plaintext", () => {
        const data = {
          user: { id: "user123", name: "owner" },
          credentials: [],
          sessions: {},
      ...
      it("does not re-hash entries that already have hash+salt", () => {
        // Simulate an already-migrated entry
        const base = AuthState.empty("user123").addSetupToken(makeTokenData());
        const stored = base.setupTokens[0];

        // Round-trip through fromJSON
        const roundTripped = AuthState.fromJSON(base.toJSON());
        const rt = roundTripped.setupTokens[0];

        // hash and salt must be preserved unchanged
        assert.strictEqual(rt.hash, stored.hash);
        assert.strictEqual(rt.salt, stored.salt);

        // Token must still be findable
        const found = roundTripped.findSetupToken("plaintext-token-value", Date.now());
        assert.ok(found);
      });

      it("migrates very old scalar setupToken field to hashed entry (always-fail sentinel)", () => {
        // Very old format: single scalar `setupToken` field, not an array
        const data = {
          user: { id: "user123", name: "owner" },
          credentials: [],
          sessions: {},
          setupToken: "ancient-scalar-token",
          // no setupTokens array
        };

        const state = AuthState.fromJSON(data);

        assert.strictEqual(state.setupTokens.length, 1, "should create one token entry");
        const stored = state.setupTokens[0];
        assert.ok(stored.hash, "sentinel entry should have a hash");
        assert.ok(stored.salt, "sentinel entry should have a salt");
        assert.strictEqual(stored.token, undefined, "plaintext must not be stored");
        // The sentinel always fails — original value is unrecoverable from the old format
        assert.strictEqual(stored.name, "Migrated Token");
      });
    });

    describe("toJSON does not expose plaintext token", () => {
      it("serialized state has no plaintext token field in setupTokens", () => {
        const state = AuthState.empty("user123").addSetupToken(makeTokenData());
        const json = state.toJSON();

        for (const t of json.setupTokens) {
          assert.strictEqual(t.token, undefined, "toJSON must not include plaintext token");
          assert.ok(t.hash, "toJSON must include hash");
          assert.ok(t.salt, "toJSON must include salt");
        }
      });
    });
  });
